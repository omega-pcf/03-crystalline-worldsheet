...
\eqref{eq:kk-BF}, so the discrete tower is stable.  (the spectrum of the Dirichlet \Proof{KK_BF_bound}
second-difference operator enters as ) \Hyp{}
\Proof{kk\_hopping\_reciprocal}%
\Proof{kk\_discrete\_spectrum, kk\_discrete\_positive, kk\_above\_BF}%
\end{proposition}
\begin{proof}
...
$\golden^{2}+\golden^{-2}=3$ enters. \Proof{kk_rowsum_eq_numerator, kk_continuous_below_BF, kk_reciprocal_discriminates}
...
mechanism but the explicit mode expansion realising it. \Proof{ladder_dim_eq_kissing_plus_rank, fks_ladder_dims, a2_minimal_vectors_exactly_six, Tdual_selfdual}
...
the classical input.  (the rearrangement enters as \Hyp{}) \Proof{hurwitz_reindex}
...
$L(6,\chi_5)=\sqrt5\pi^{6}\cdot\tfrac{536}{1171875}$.  (Bernoulli enters as \Hyp{}) \Proof{even_L_rationality, bernoulli_eval_rational}
