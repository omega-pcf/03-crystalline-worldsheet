...
de~Sitter embeds. \P{ets\_constant\_block\_christoffels\_vanish}
...
$K_{\mu\nu}=(\lambda/\sigma^2)g_{\mu\nu}$. \P{ets\_slices\_totally\_geodesic}
...
$\tfrac34+\tfrac14=1$ \P{casimir\_plus\_newton}
and the boundary density ratio is
$\golden^{-2}-1=-\golden^{-1}$ \P{boundary\_density\_ratio}
.
...
\eqref{eq:kk-BF}, so the discrete tower is stable.  (the spectrum of the Dirichlet \P{KK_BF_bound}
second-difference operator enters as ) \Hyp{}
\P{kk\_hopping\_reciprocal}%
\P{kk\_discrete\_spectrum, kk\_discrete\_positive, kk\_above\_BF}%
\end{proposition}
\begin{proof}
...
bound \eqref{eq:kk-BF} because $\ln\golden<\tfrac12$ \P{log\_phi\_lt\_half}
Truncating to the
...
$\golden^{2}+\golden^{-2}=3$ enters. \P{kk_rowsum_eq_numerator, kk_continuous_below_BF, kk_reciprocal_discriminates}
...
mechanism but the explicit mode expansion realising it. \P{ladder_dim_eq_kissing_plus_rank, fks_ladder_dims, a2_minimal_vectors_exactly_six, Tdual_selfdual}
...
the classical input.  (the rearrangement enters as \Hyp{}) \P{hurwitz_reindex}
...
$L(6,\chi_5)=\sqrt5\pi^{6}\cdot\tfrac{536}{1171875}$.  (Bernoulli enters as \Hyp{}) \P{even_L_rationality, bernoulli_eval_rational}
...
(Gauss--Binet enters as \Hyp{}) \P{}
\P{kappaK\_two\_real\_places, kappaK\_alt, kappaK\_simple\_pole, kappaK\_pole\_numerator\_at\_one,
