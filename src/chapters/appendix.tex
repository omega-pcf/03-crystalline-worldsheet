...
\Lean{ricciCoeff, ricciScalar, einsteinCoeff, R\_AB\_einstein, R\_scalar\_pcf, G\_AB\_pcf}
...
de~Sitter embeds. \Lean{ets\_constant\_block\_christoffels\_vanish}
...
\Lean{ladder\_dim\_eq\_kissing\_plus\_rank, a2\_roots\_give\_su3\_dim, gauge\_dim\_su3, weinberg\_angle\_gut, Nmodes\_zero\_eq\_three, Nmodes\_two\_eq\_eight, entropy\_ratio\_S3\_S6, beta\_is\_mssm, hopf\_from\_clifford, G\_Lambda\_duality}
...
\eqref{eq:kk-BF}, so the discrete tower is stable. \tier{P} (the spectrum of the Dirichlet
second-difference operator enters as \tier{L})
\LeanFor{eq:kk-operator}{kk\_hopping\_reciprocal}%
\LeanFor{eq:kk-spectrum}{kk\_discrete\_spectrum, kk\_discrete\_positive, kk\_above\_BF}%
\end{proposition}
\begin{proof}
...
bound \eqref{eq:kk-BF} because $\ln\golden<\tfrac12$ \Lean{log\_phi\_lt\_half}. Truncating to the
...
$\golden^{2}+\golden^{-2}=3$ enters. Both are checked in
\texttt{CW6\_complete\_verify\_v2.py}. \tier{P/N}
\Lean{kk\_rowsum\_eq\_numerator, kk\_continuous\_below\_BF, kk\_reciprocal\_discriminates}
\end{remark}
...
$R^{2}=\alpha'$\,\Lean{Tdual\_selfdual}, where the minimal vectors of the lattice supply the root
...
mechanism but the explicit mode expansion realising it. \tier{P}
\Lean{ladder\_dim\_eq\_kissing\_plus\_rank, fks\_ladder\_dims, a2\_minimal\_vectors\_exactly\_six, Tdual\_selfdual}
...
the classical input. \tier{P} (the rearrangement enters as \tier{L}) \Lean{hurwitz\_reindex}
...
$L(6,\chi_5)=\sqrt5\pi^{6}\cdot\tfrac{536}{1171875}$. \tier{P} (Bernoulli enters as \tier{L})
\Lean{even\_L\_rationality, bernoulli\_eval\_rational}
...
\tier{P} (Gauss--Binet enters as \tier{L})
\Lean{kappaK\_two\_real\_places, kappaK\_alt, kappaK\_simple\_pole, kappaK\_pole\_numerator\_at\_one,
kappaK\_pos, kappaK\_exp, kappa\_per\_real\_place}

% ===================================================================
\section{Translation of the epigraph (Schiller, 1785)}\label{app:schiller}
The German verse quoted in the acknowledgements is the opening strophe of Friedrich
Schiller's \emph{An die Freude} (``To Joy'', 1785), the poem later set to music by Ludwig
van Beethoven in the final movement of his Ninth Symphony (\emph{Ode an die Freude},
Op.~125, 1824). A standard English translation runs:
...
\end{quote}
\noindent--- Friedrich Schiller, \emph{An die Freude} (1785), English rendering.
