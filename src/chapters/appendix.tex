...
de~Sitter embeds. \Proof{ets\_constant\_block\_christoffels\_vanish}
...
$K_{\mu\nu}=(\lambda/\sigma^2)g_{\mu\nu}$. \Proof{ets\_slices\_totally\_geodesic}
...
$\tfrac34+\tfrac14=1$ \Proof{casimir\_plus\_newton}
and the boundary density ratio is
$\golden^{-2}-1=-\golden^{-1}$ \Proof{boundary\_density\_ratio}
.
...
\eqref{eq:kk-BF}, so the discrete tower is stable.  (the spectrum of the Dirichlet \Proof{KK\_BF\_bound}
second-difference operator enters as ) \Hyp{}
\ProofBlock{kk\_hopping\_reciprocal}%
\ProofBlock{kk\_discrete\_spectrum, kk\_discrete\_positive, kk\_above\_BF}%
\end{proposition}
\begin{proof}
...
bound \eqref{eq:kk-BF} because $\ln\golden<\tfrac12$ \Proof{log\_phi\_lt\_half}
Truncating to the
...
$\golden^{2}+\golden^{-2}=3$ enters. \Proof{kk\_rowsum\_eq\_numerator, kk\_continuous\_below\_BF, kk\_reciprocal\_discriminates}
...
$R^{2}=\alpha'$\,\Proof{Tdual\_selfdual}
...
mechanism but the explicit mode expansion realising it. \Proof{ladder\_dim\_eq\_kissing\_plus\_rank, fks\_ladder\_dims, a2\_minimal\_vectors\_exactly\_six, Tdual\_selfdual}
...
the classical input.  (the rearrangement enters as \Hyp{}) \Proof{hurwitz\_reindex}
...
$L(6,\chi_5)=\sqrt5\pi^{6}\cdot\tfrac{536}{1171875}$.  (Bernoulli enters as \Hyp{}) \Proof{even\_L\_rationality, bernoulli\_eval\_rational}
...
(Gauss--Binet enters as \Hyp{}) \Proof{kappaK\_two\_real\_places, kappaK\_alt, kappaK\_simple\_pole, kappaK\_pole\_numerator\_at\_one,
