...
de~Sitter embeds. \ProofWitness{ets\_constant\_block\_christoffels\_vanish}
...
$K_{\mu\nu}=(\lambda/\sigma^2)g_{\mu\nu}$. \ProofWitness{ets\_slices\_totally\_geodesic}
...
$\tfrac34+\tfrac14=1$ \ProofWitness{casimir\_plus\_newton}
and the boundary density ratio is
$\golden^{-2}-1=-\golden^{-1}$ \ProofWitness{boundary\_density\_ratio}
.
...
\HypWitness{the spectrum of the Dirichlet second-difference operator enters as hypothesis}
...
bound \eqref{eq:kk-BF} because $\ln\golden<\tfrac12$ \ProofWitness{log\_phi\_lt\_half}
Truncating to the
...
$\golden^{2}+\golden^{-2}=3$ enters. \ProofWitness{kk\_rowsum\_eq\_numerator, kk\_continuous\_below\_BF, kk\_reciprocal\_discriminates}
...
mechanism but the explicit mode expansion realising it. \ProofWitness{ladder\_dim\_eq\_kissing\_plus\_rank, fks\_ladder\_dims, a2\_minimal\_vectors\_exactly\_six, Tdual\_selfdual}
...
the classical input.  (the rearrangement enters as \HypWitness{}) \ProofWitness{hurwitz\_reindex}
...
$L(6,\chi_5)=\sqrt5\pi^{6}\cdot\tfrac{536}{1171875}$.  (Bernoulli enters as \HypWitness{}) \ProofWitness{even\_L\_rationality, bernoulli\_eval\_rational}
...
(Gauss--Binet enters as \HypWitness{}) \ProofWitness{kappaK\_two\_real\_places, kappaK\_alt, kappaK\_simple\_pole, kappaK\_pole\_numerator\_at\_one,
