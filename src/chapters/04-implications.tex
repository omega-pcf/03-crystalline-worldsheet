...
technical classification: type~I as discrete spectrum (\P{eigenvalues\_modulus\_half, witten\_finite\_rank}); type~II as an invariant the flow preserves---the trace in
concrete form (\P{omega\_flow\_invariant, tower\_scale\_group}); type~III as the
...
separate (\P{fibre\_independent\_of\_base}). By the modulus $\tfrac12$, the loop
...
\P{projection\_closed\_form, M8\_epsilon0\_from\_projection}%
...
every scale, $|\Om|_\sigma=\tfrac12\ \forall\sigma$ (\P{omega\_flow\_invariant}). At
...
\P{bridge\_cocycle, bridge\_inverse, bridge\_compose}).
...
\P{cpt\_swaps\_P\_to\_F, cpt\_fixes\_C, cpt\_involution, cpt\_preserves\_modulus}.
...
$|\Om|_\sigma=\tfrac12$. \emph{Proof:} \P{eisenstein\_cube, omega\_conj}; \cite{Witten01dS}.
...
(\S\ref{app:curvature}; \P{R\_scalar\_pcf, R\_AB\_einstein}). The densification of
...
\S\ref{app:embedding} (\P{dS\_ricci\_from\_gauss}).
...
(\P{M2\_gamma\_half}), computed where it is stated. The particle is then a type~I
...
(\S\ref{app:gauge}; \P{gauge\_dim\_su3, hopf\_from\_clifford, weinberg\_angle\_gut}).
...
\P{cw3\_observer\_items\_core}; \cite{CLPW22}.
...
integer. \emph{Proof:} \P{witten\_finite\_rank}; \eqref{eq:obs-spinstar}.
...
as the time axis, its square changes sign, $(i\,t)^2=-t^2$ (\P{wick\_squares\_flip\_sign, lorentzian\_signature\_sum}), and past and future become the two light cones, with the
apex---the present---as observer (\P{C\_fixed\_PF\_rotate, causal\_trichotomy}). The
...
$|\Om|=|\Om|^{2}=1$, $H=0$  (the Schmidt decomposition enters as \Hyp{}) . Hence before observation there are no separable subsystems carrying \P{no_separable_parts, separable_iff_top_eigenvalue_one}
...
(\P{binary\_entropy\_half}), is the maximally mixed state---$\rho_{\max}=1$ in CLPW's
...
\eqref{eq:certainty}, here as \P{eps0\_Mpcf\_pi}, and checked numerically; the equivalence itself is
...
$S=\ln N$. \emph{Proof:}
\P{witten\_entropy\_realized} (by \texttt{rfl}---the most direct realisation),
\P{witten\_finite\_rank, witten\_two\_conjectures}.
The entropy
...
flow of the static patch at the Gibbons--Hawking temperature (\P{tower\_scale\_group, tower\_flow\_fixes\_tau, omega\_flow\_group, omega\_flow\_invariant}).
...
\P{omega\_flow\_invariant, tower\_scale\_group} \cite{CLPW22}.
...
\P{binary\_entropy\_half} \cite{CLPW22}.
...
(Remark~\ref{rmk:residual}). \P{lattice_radius_conjugate}
...
\P{fisher\_unit\_iff, H\_half\_one\_bit}%
...
for the arithmetic; \Hyp{} for the junction condition itself~\cite{Israel66}. \P{israel_prefactor, israel_jump_determined}
...
number of bits the level carries. \P{backreaction_cumulative} \N{eq:shell-tension}
\end{remark}
...
$\golden^{-6}\cdot\golden^{6}=1$ (\P{G\_Lambda\_duality, R\_scalar\_pcf, BF\_value}).
...
continuation rather than an imposed duality (\P{ets\_is\_dS\_zero\_H}). Past and future
...
flat---its curvature, computed, vanishes, $R^{(5)}=0$ (\P{ets\_constant\_block\_christoffels\_vanish})---and the
...
(\P{dS\_ricci\_from\_gauss, dS\_einstein\_Lambda}). The planar patch covers exactly half
the hyperboloid---$X_0+X_4=\ell\,e^{t/\ell}>0$ never changes sign
(\P{dS\_covers\_half\_hyperboloid})---and that half \emph{realises} the observer's half
$|\Om|=\tfrac12$ by the shared value (\P{observer\_half\_from\_norms}). The assertoric
...
\emph{Proof:} \P{dS\_temperature, dS\_beta\_reciprocal} (\S\ref{app:embedding}) \cite{GibbonsHawking77}.
...
\P{binary_entropy_half, S_tower_recurrence} \N{prop:entropy-max}
\end{proposition}
...
\P{gauge\_dim\_su3, phi\_central\_chain, central\_chain, KK\_numerator, clifford\_S3\_condition, hopf\_from\_clifford, a2\_minimal\_vectors\_exactly\_six}.

The electroweak data follow from the same modulus: the weak mixing angle at unification is
$\sin^{2}\theta_W|_{\rm GUT}=\tfrac38=N(0)/N(2)$ \P{weinberg\_angle\_gut}
running to $0.231$ at
$M_Z$; the tower entropy ratio $S(3)/S(6)=\golden^{-3}$ \P{entropy\_ratio\_S3\_S6} marks the
...
(the vanishing-$00$ and spatial values are \N{}, symbolic) \P{dS_first_law, energy_is_inv_H, komar_first_law, mu3_temp_ratio, LL_spatial_ne_zero}
...
ladder rather than a continuum. \N{}
\P{lambda\_magnitude\_eq\_level}%
\P{sigma\_G\_val, sigma\_EM\_val, sigma\_Lambda\_val, threshold\_ordering}%
\P{interval\_gap, interval\_gap\_general}%
\P{em\_holographic\_fraction, em\_asymmetry}%
\P{entropy\_at\_mu\_three, tower\_step\_ratio}%
\end{remark}
...
the angle is then evaluated on them. \P{interval_uniqueness, interval_arity_discriminates, interval_gap_only_three}
...
level, the Fibonacci quantum dimension (Corollary~\ref{cor:golden-growth}).  (six checks); \N{cor:golden-growth}
D3 is \P{binary_entropy_half}
...
Fig.~\ref{fig:tower-modes}. \P{S_tower, tower_step_ratio}
...
eigenvalues equal $\tfrac12=\lvert\Om\rvert$. \P{projector_idem, rho_is_state, rho_k2_eq_half_projector, projector_trace_eq_rank}
...
lets the state reading and the amplitude reading be taken on one object. \P{two_positivities_distinct, rho_k2_eq_half_projector}
...
(\S\ref{ssec:amp}). \P{projector_frame_invariant}
%
\P{projector\_idem, projector\_trace\_eq\_rank, rho\_is\_state, positroid\_from\_kp}%
\end{theorem}
\begin{proof}
...
dimension count is \P{ladder_from_scott, scott_kissing_eq_two_posroots, kissing_eq_two_posroots, dims_even_ladder, ladder_dim_eq_kissing_plus_rank, fks_ladder_dims}
...
(\S\ref*{ssec:p-tower}, \eqref{eq:euler-product}). \P{veneziano_eq_gamma_ratio, regge_tower_is_euler_product}
...
canonical form (the $\golden$ side). \P{ft_identity, ft_limit, pentagon_u_eq_golden}
\end{proposition}
...
tower's own clock.
; the double copy is \Hyp{} (Bern--Carrasco--Johansson) \P{fisher_time_lock, entropy_response_per_bit, entropy_response_hasDerivAt}
...
\end{equation}
reproducing the MSSM one-loop coefficients with no superpartner particles. \P{mssm_eq_sm_plus_bridges, bridge_cocycle}
...
the decomposition of the identity that the groupoid of \S\ref{ssec:hinge} composes. \P{er_epr_state_eq_geometry, er_epr_bridge}
...
independent: one sums representations, the other sums bridges. \P{bridges_reproduce_mssm, beta_is_mssm}
...
the bridges of \S\ref{ssec:susy}.  (Kodama--Williams enters as \Hyp{}) \P{positroid\_from\_kp, bridge\_cocycle} \N{}
\end{proposition}
...
with the vacuum at $0$. \P{towerHam_isHermitian, casimirOp_isHermitian, towerHam_bounded_below, casimirOp_bounded_below}
...
tower's own spacing, the other the colour energy. \P{spectrum_discrete, vacuum_unique, mass_gap, operators_selfadjoint_and_isometry}
...
(the GKPW dictionary itself enters as \Hyp{}) \P{towerE_eq_exp_regulator, S_tower_eq_exp_regulator}
%
\P{modular\_bulk\_ratio\_level\_independent, bulk\_boundary\_intertwine}%
\end{theorem}
\begin{proof}
...
arbitrary. \P{theta_pairing_eq_transfer, reflection_positive_general, half_prop_sq, theta_involutive, theta_preserves_modulus_C, theta_commutes_real_diagonal, transfer_positive, cpt_involution, cpt_preserves_modulus}
...
\P{reflCov_factors, reflQuadForm_eq_square, reflection_positive_measure}
\end{proposition}
\begin{proof}
...
Osterwalder--Schrader axioms. \P{os_reconstruction_explicit, transfer_spectral_data, clustering_exponential, vacuum_transfer_eq_one, transfer_gap_below_vacuum}
...
\P{os_reconstruction_explicit, towerHam_isHermitian, towerHam_bounded_below}
\end{corollary}
...
(\S\ref*{ssec:p-tower}) it is the analyticity/temperedness axiom. \P{internal_wick}
\end{remark}
...
gauge theory. \P{bulkBoundary_isometry}
\end{theorem}
\begin{proof}
...
(\S\ref{ssec:susy}). \P{regge_tower_is_euler_product, regge_dirichlet_eq_zeta} \N{prop:veneziano}
...
under the duality that relates strong and weak coupling. ; the dual-superconductor mechanism \N{}
...
$V(\sigma)=\epsz\golden^{-\sigma}$\,\P{swampland\_hasDerivAt}
...
fixed level by level rather than free.  (the level-invariance $\sigma_{\mathrm{tension}}(\sigma)\,S(\sigma)=4\pi^4/(q^2\Mpcf)$ is proved: the $\golden^{\sigma}$ of the entropy cancels the $\golden^{-\sigma}$ of the condensate) \P{swampland_hasDerivAt}
%
\P{eps0\_Mpcf\_pi, tension\_welded\_to\_tower, tension\_weld\_value}%
\end{proposition}
...
for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \Hyp{} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \P{modular_generator_is_entropy, modular_generator_is_LL_over_T, modular_generator_is_tower, scale_flow_is_dilation, hubble_descends_tower}
\end{theorem}
...
(data comparison only) \P{M\_eq\_Mpcf} \Hyp{}
\end{remark}
...
Theorem~\ref{thm:zeta-odd}, and the character Proposition~\ref{prop:pentagon-chi5}. \P{s_duality_exact, functional_equation_fixed_at_half, places_assembly}
...
theorem (Theorem~\ref{thm:funct-eq}, Theorem~\ref{thm:duality}). \P{fixed_line_is_facePhi, functional_equation_fixed_line, places_assembly, s_duality_exact, s_duality_fixed_point_unique, theta_from_jacobi, functional_equation_fixed_at_half, places_assembly, s_duality_exact}
...
Proposition~\ref{prop:fluid}). \P{er_epr_bridge, er_epr_state_eq_geometry, bridge_cocycle, positroid_from_kp}
...
is not the number but the resolution of the spin structures onto it. \Hyp{} (bridge) \P{framework_mass_gap, gap_survives, eps0_Mpcf_pi}
...
the section: each can be checked on its own without the surrounding reading. \P{entropy_dof_core, gauge_side_facts, transfer_spectral_data}
