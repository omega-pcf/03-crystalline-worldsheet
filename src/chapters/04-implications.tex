...
number of bits the level carries. \ProofWitnessBlock{backreaction\_cumulative} \NumericalWitnessBlock{eq:shell-tension}
\end{remark}
...
(the vanishing-$00$ and spatial values are \NumericalWitnessBlock{eq:obs-landauer}, symbolic) \ProofWitnessBlock{dS\_first\_law, energy\_is\_inv\_H, komar\_first\_law, mu3\_temp\_ratio, LL\_spatial\_ne\_zero}
...
the bridges of \S\ref{ssec:susy}.  (Kodama--Williams enters as \HypWitness{\cite{KodamaWilliams}}) \ProofWitnessBlock{positroid\_from\_kp, bridge\_cocycle} \NumericalWitnessBlock{TODO}
\end{proposition}
...
gauge theory. \ProofWitnessBlock{bulkBoundary\_isometry}
\end{theorem}
\begin{proof}
...
for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \HypWitness{\cite{BisognanoWichmann75}} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \ProofWitnessBlock{modular\_generator\_is\_entropy, modular\_generator\_is\_LL\_over\_T, modular\_generator\_is\_tower, scale\_flow\_is\_dilation, hubble\_descends\_tower}
\end{theorem}
