...
$\sin^{2}\theta_W|_{\rm GUT}=\tfrac38=N(0)/N(2)$ \Lean{weinberg\_angle\_gut}, running to $0.231$ at
$M_Z$; the tower entropy ratio $S(3)/S(6)=\golden^{-3}$ \Lean{entropy\_ratio\_S3\_S6} marks the
...
D3 is \tier{P}
(\S\ref*{ssec:construction}, \Lean{binary\_entropy\_half}).
...
$V(\sigma)=\epsz\golden^{-\sigma}$\,\Lean{swampland\_hasDerivAt}. The two are therefore conjugate,
...
\item the theory sits at an ultraviolet fixed point, the asymptotic-safety corner of the duality web
      ...
      (\S\ref{ssec:accum})\,\Lean{c\_tendsto\_two};
\item and that fixed point is not tuned: Assumption~\ref{asm:bridge} (\eqref{eq:obs-fixedpoint}) makes it \emph{equivalent} to the
      ...
      accumulated once around the fundamental domain\,\Lean{eps0\_Mpcf\_pi}.
\end{itemize}
