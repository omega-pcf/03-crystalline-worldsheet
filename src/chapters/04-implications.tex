...
technical classification: type~I as discrete spectrum (\Proof{eigenvalues\_modulus\_half, witten\_finite\_rank}); type~II as an invariant the flow preserves---the trace in
concrete form (\Proof{omega\_flow\_invariant, tower\_scale\_group}); type~III as the
...
separate (\Proof{fibre\_independent\_of\_base}). By the modulus $\tfrac12$, the loop
...
\ProofBlock{projection\_closed\_form, M8\_epsilon0\_from\_projection}%
...
\ProofBlock{omega\_flow\_invariant, bridge\_cocycle, bridge\_inverse, bridge\_compose}
\end{proposition}
Three independent facts confirm the non-separability, and all three fall on $\tfrac12$. At
every scale, $|\Om|_\sigma=\tfrac12\ \forall\sigma$ (\Proof{omega\_flow\_invariant}). At
...
\ProofBlock{bridge\_cocycle, bridge\_inverse, bridge\_compose}).
...
\ProofBlock{cpt\_swaps\_P\_to\_F, cpt\_fixes\_C, cpt\_involution, cpt\_preserves\_modulus}.
...
$|\Om|_\sigma=\tfrac12$. \emph{Proof:} \Proof{eisenstein\_cube, omega\_conj}; \cite{Witten01dS}.
...
(\S\ref{app:curvature}; \Proof{R\_scalar\_pcf, R\_AB\_einstein}). The densification of
...
\S\ref{app:embedding} (\Proof{dS\_ricci\_from\_gauss}).
...
(\Proof{M2\_gamma\_half}), computed where it is stated. The particle is then a type~I
...
(\S\ref{app:gauge}; \Proof{gauge\_dim\_su3, hopf\_from\_clifford, weinberg\_angle\_gut}).
...
\ProofBlock{cw3\_observer\_items\_core}; \cite{CLPW22}.
...
integer. \emph{Proof:} \Proof{witten\_finite\_rank}; \eqref{eq:obs-spinstar}.
...
as the time axis, its square changes sign, $(i\,t)^2=-t^2$ (\Proof{wick\_squares\_flip\_sign, lorentzian\_signature\_sum}), and past and future become the two light cones, with the
apex---the present---as observer (\Proof{C\_fixed\_PF\_rotate, causal\_trichotomy}). The
...
$|\Om|=|\Om|^{2}=1$, $H=0$  (the Schmidt decomposition enters as \Hyp{}) . Hence before observation there are no separable subsystems carrying \Proof{no\_separable\_parts, separable\_iff\_top\_eigenvalue\_one}
...
(\Proof{binary\_entropy\_half}), is the maximally mixed state---$\rho_{\max}=1$ in CLPW's
...
\eqref{eq:certainty}, here as \Proof{eps0\_Mpcf\_pi}, and checked numerically; the equivalence itself is
...
$S=\ln N$. \emph{Proof:}
\ProofBlock{witten\_entropy\_realized} (by \texttt{rfl}---the most direct realisation),
\ProofBlock{witten\_finite\_rank, witten\_two\_conjectures}.
The entropy
...
flow of the static patch at the Gibbons--Hawking temperature (\Proof{tower\_scale\_group, tower\_flow\_fixes\_tau, omega\_flow\_group, omega\_flow\_invariant}).
...
\ProofBlock{omega\_flow\_invariant, tower\_scale\_group} \cite{CLPW22}.
...
\ProofBlock{binary\_entropy\_half} \cite{CLPW22}.
...
(Remark~\ref{rmk:residual}). \Proof{lattice\_radius\_conjugate}
...
\ProofBlock{fisher\_unit\_iff, H\_half\_one\_bit}%
...
\ProofBlock{energy\_per\_bit\_constant}
\end{proposition}
\begin{proof}
...
\end{equation}
\ProofBlock{shell\_tension\_determined}
\end{proposition}
\begin{proof}
...
for the arithmetic; \Hyp{} for the junction condition itself~\cite{Israel66}. \Proof{israel\_prefactor, israel\_jump\_determined}
...
number of bits the level carries. \Proof{backreaction\_cumulative} \Numeric{eq:shell-tension}
\end{remark}
...
$\golden^{-6}\cdot\golden^{6}=1$ (\Proof{G\_Lambda\_duality, R\_scalar\_pcf, BF\_value}).
...
continuation rather than an imposed duality (\Proof{ets\_is\_dS\_zero\_H}). Past and future
...
flat---its curvature, computed, vanishes, $R^{(5)}=0$ (\Proof{ets\_constant\_block\_christoffels\_vanish})---and the
...
(\Proof{dS\_ricci\_from\_gauss, dS\_einstein\_Lambda}). The planar patch covers exactly half
the hyperboloid---$X_0+X_4=\ell\,e^{t/\ell}>0$ never changes sign
(\Proof{dS\_covers\_half\_hyperboloid})---and that half \emph{realises} the observer's half
$|\Om|=\tfrac12$ by the shared value (\Proof{observer\_half\_from\_norms}). The assertoric
...
\emph{Proof:} \Proof{dS\_temperature, dS\_beta\_reciprocal} (\S\ref{app:embedding}) \cite{GibbonsHawking77}.
...
\ProofBlock{binary\_entropy\_half, S\_tower\_recurrence} \Numeric{prop:entropy-max}
\end{proposition}
...
\ProofBlock{G\_AB\_pcf, R\_AB\_einstein, R\_scalar\_pcf, einsteinCoeff, ricciCoeff, ricciScalar, BF\_value}
\end{proposition}
...
\ProofBlock{dS\_einstein\_Lambda, R\_AB\_einstein, G\_AB\_pcf}
\end{theorem}
...
\ProofBlock{gauge\_dim\_su3, phi\_central\_chain, central\_chain, KK\_numerator, clifford\_S3\_condition, hopf\_from\_clifford, a2\_minimal\_vectors\_exactly\_six}.

The electroweak data follow from the same modulus: the weak mixing angle at unification is
$\sin^{2}\theta_W|_{\rm GUT}=\tfrac38=N(0)/N(2)$ \Proof{weinberg\_angle\_gut}
running to $0.231$ at
$M_Z$; the tower entropy ratio $S(3)/S(6)=\golden^{-3}$ \Proof{entropy\_ratio\_S3\_S6} marks the
...
(the vanishing-$00$ and spatial values are \Numeric{eq:obs-landauer}, symbolic) \Proof{dS\_first\_law, energy\_is\_inv\_H, komar\_first\_law, mu3\_temp\_ratio, LL\_spatial\_ne\_zero}
...
\ProofBlock{two\_routes\_to\_six}%
...
ladder rather than a continuum. \Numeric{eq:Lambda-from-curvature}
\ProofBlock{lambda\_magnitude\_eq\_level}%
\ProofBlock{sigma\_G\_val, sigma\_EM\_val, sigma\_Lambda\_val, threshold\_ordering}%
\ProofBlock{interval\_gap, interval\_gap\_general}%
\ProofBlock{em\_holographic\_fraction, em\_asymmetry}%
\ProofBlock{entropy\_at\_mu\_three, tower\_step\_ratio}%
\end{remark}
...
the angle is then evaluated on them. \Proof{interval\_uniqueness, interval\_arity\_discriminates, interval\_gap\_only\_three}
...
level, the Fibonacci quantum dimension (Corollary~\ref{cor:golden-growth}).  (six checks); \Numeric{cor:golden-growth}
D3 is \Proof{binary\_entropy\_half}
...
Fig.~\ref{fig:tower-modes}. \Proof{S\_tower, tower\_step\_ratio}
...
eigenvalues equal $\tfrac12=\lvert\Om\rvert$. \Proof{projector\_idem, rho\_is\_state, rho\_k2\_eq\_half\_projector, projector\_trace\_eq\_rank}
...
lets the state reading and the amplitude reading be taken on one object. \Proof{two\_positivities\_distinct, rho\_k2\_eq\_half\_projector}
...
(\S\ref{ssec:amp}). \Proof{projector\_frame\_invariant}
%
\ProofBlock{projector\_idem, projector\_trace\_eq\_rank, rho\_is\_state, positroid\_from\_kp}%
\end{theorem}
\begin{proof}
...
\ProofBlock{eisenstein, Omega\_eigenvalues, su3\_dim}
\end{proposition}
\begin{proof}
...
combinatorial ladder is, rung by rung, the ladder of gauge groups (\S\ref{app:kk}). ; the \Hyp{}
dimension count is \Proof{ladder\_from\_scott, scott\_kissing\_eq\_two\_posroots, kissing\_eq\_two\_posroots, dims\_even\_ladder, ladder\_dim\_eq\_kissing\_plus\_rank, fks\_ladder\_dims}
...
(\S\ref*{ssec:p-tower}, \eqref{eq:euler-product}). \Proof{veneziano\_eq\_gamma\_ratio, regge\_tower\_is\_euler\_product}
...
canonical form (the $\golden$ side). \Proof{ft\_identity, ft\_limit, pentagon\_u\_eq\_golden}
\end{proposition}
...
\ProofBlock{beta\_is\_mssm, sm\_anomaly\_free, weinberg\_angle\_gut}
\end{proposition}
...
tower's own clock.
; the double copy is \Hyp{} (Bern--Carrasco--Johansson) \Proof{fisher\_time\_lock, entropy\_response\_per\_bit, entropy\_response\_hasDerivAt}
...
\end{equation}
reproducing the MSSM one-loop coefficients with no superpartner particles. \Proof{mssm\_eq\_sm\_plus\_bridges, bridge\_cocycle}
...
the decomposition of the identity that the groupoid of \S\ref{ssec:hinge} composes. \Proof{er\_epr\_state\_eq\_geometry, er\_epr\_bridge}
...
independent: one sums representations, the other sums bridges. \Proof{bridges\_reproduce\_mssm, beta\_is\_mssm}
...
the bridges of \S\ref{ssec:susy}.  (Kodama--Williams enters as \Hyp{}) \Proof{positroid\_from\_kp, bridge\_cocycle} \Numeric{TODO}
\end{proposition}
...
with the vacuum at $0$. \Proof{towerHam\_isHermitian, casimirOp\_isHermitian, towerHam\_bounded\_below, casimirOp\_bounded\_below}
...
tower's own spacing, the other the colour energy. \Proof{spectrum\_discrete, vacuum\_unique, mass\_gap, operators\_selfadjoint\_and\_isometry}
...
(the GKPW dictionary itself enters as \Hyp{}) \Proof{towerE\_eq\_exp\_regulator, S\_tower\_eq\_exp\_regulator}
%
\ProofBlock{modular\_bulk\_ratio\_level\_independent, bulk\_boundary\_intertwine}%
\end{theorem}
\begin{proof}
...
What is \emph{not} proved here is the GKPW dictionary itself; it enters cited, \Hyp{}
...
\begin{equation}\label{eq:gap}
  \Delta=\tfrac23 g^2>0. 
\end{equation}
\ProofBlock{strong\_coupling\_gap}
\end{theorem}
\begin{proof}
...
arbitrary. \Proof{theta\_pairing\_eq\_transfer, reflection\_positive\_general, half\_prop\_sq, theta\_involutive, theta\_preserves\_modulus\_C, theta\_commutes\_real\_diagonal, transfer\_positive, cpt\_involution, cpt\_preserves\_modulus}
...
\ProofBlock{reflCov\_factors, reflQuadForm\_eq\_square, reflection\_positive\_measure}
\end{proposition}
\begin{proof}
...
Osterwalder--Schrader axioms. \Proof{os\_reconstruction\_explicit, transfer\_spectral\_data, clustering\_exponential, vacuum\_transfer\_eq\_one, transfer\_gap\_below\_vacuum}
...
\ProofBlock{os\_reconstruction\_explicit, towerHam\_isHermitian, towerHam\_bounded\_below}
\end{corollary}
...
(\S\ref*{ssec:p-tower}) it is the analyticity/temperedness axiom. \Proof{internal\_wick}
\end{remark}
...
gauge theory. \Proof{bulkBoundary\_isometry}
\end{theorem}
\begin{proof}
...
(\S\ref{ssec:susy}). \Proof{regge\_tower\_is\_euler\_product, regge\_dirichlet\_eq\_zeta} \Numeric{prop:veneziano}
...
under the duality that relates strong and weak coupling. ; the dual-superconductor mechanism \Numeric{eq:string-tension}
is  ('t~Hooft--Mandelstam). \Hyp{}
\end{proposition}
...
$V(\sigma)=\epsz\golden^{-\sigma}$\,\Proof{swampland\_hasDerivAt}
...
fixed level by level rather than free.  (the level-invariance $\sigma_{\mathrm{tension}}(\sigma)\,S(\sigma)=4\pi^4/(q^2\Mpcf)$ is proved: the $\golden^{\sigma}$ of the entropy cancels the $\golden^{-\sigma}$ of the condensate) \Proof{swampland\_hasDerivAt}
%
\ProofBlock{eps0\_Mpcf\_pi, tension\_welded\_to\_tower, tension\_weld\_value}%
\end{proposition}
...
\ProofBlock{S\_is\_operator\_spectrum, tension\_is\_conjugate\_spectrum, tension\_welded\_to\_tower, mass\_gap}
\end{proposition}
...
for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \Hyp{} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \Proof{modular\_generator\_is\_entropy, modular\_generator\_is\_LL\_over\_T, modular\_generator\_is\_tower, scale\_flow\_is\_dilation, hubble\_descends\_tower}
\end{theorem}
...
\ProofBlock{M\_eq\_Mpcf, colour\_scale\_from\_M, colour\_running\_fixed}
\end{theorem}
...
(data comparison only) \Proof{M\_eq\_Mpcf} \Hyp{}
\end{remark}
...
\ProofBlock{scale\_is\_modulus, one\_object}
\end{theorem}
...
Theorem~\ref{thm:zeta-odd}, and the character Proposition~\ref{prop:pentagon-chi5}. \Proof{s\_duality\_exact, functional\_equation\_fixed\_at\_half, places\_assembly}
...
\ProofBlock{gap\_independent\_of\_cutoff, eps0\_Mpcf\_pi, Lambda\_QCD\_eq\_Lambda, Lambda\_QCD\_pos} \Numeric{eq:transmutation}
\end{corollary}
...
any holographic model. \Proof{strong\_coupling\_gap}
...
theorem (Theorem~\ref{thm:funct-eq}, Theorem~\ref{thm:duality}). \Proof{fixed\_line\_is\_facePhi, functional\_equation\_fixed\_line, places\_assembly, s\_duality\_exact, s\_duality\_fixed\_point\_unique, theta\_from\_jacobi, functional\_equation\_fixed\_at\_half, places\_assembly, s\_duality\_exact}
...
Proposition~\ref{prop:fluid}). \Proof{er\_epr\_bridge, er\_epr\_state\_eq\_geometry, bridge\_cocycle, positroid\_from\_kp}
...
is not the number but the resolution of the spin structures onto it. \Hyp{} (bridge) \Proof{framework\_mass\_gap, gap\_survives, eps0\_Mpcf\_pi}
...
the section: each can be checked on its own without the surrounding reading. \Proof{entropy\_dof\_core, gauge\_side\_facts, transfer\_spectral\_data}
