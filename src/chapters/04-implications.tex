...
\LeanFor{eq:obs-interface}{projection\_closed\_form, M8\_epsilon0\_from\_projection}%
...
\tier{P} \Lean{omega\_flow\_invariant, bridge\_cocycle, bridge\_inverse, bridge\_compose}
\end{proposition}
...
\LeanInline{cpt\_swaps\_P\_to\_F, cpt\_fixes\_C, cpt\_involution, cpt\_preserves\_modulus}.
...
(\S\ref{app:curvature}; \LeanInline{R\_scalar\_pcf, R\_AB\_einstein}). The densification of
...
(\S\ref{app:gauge}; \LeanInline{gauge\_dim\_su3, hopf\_from\_clifford, weinberg\_angle\_gut}).
\begin{figure}[htb]\centering
\includegraphics[width=\linewidth]{fig6_cylinder_torus.pdf}
...
as the time axis, its square changes sign, $(i\,t)^2=-t^2$ (\LeanInline{wick\_squares\_flip\_sign, lorentzian\_signature\_sum}), and past and future become the two light cones, with the
...
$\mu=\tfrac12$, $\sigma=\tfrac32$; \LeanWrap{spectral\_uniqueness}).
...
remains. \emph{Proof:} \LeanWrap{pairing\_FP\_modulus\_half, pairing\_PF\_modulus\_half}.
...
\emph{Proof:} \LeanWrap{witten\_unitarity\_realized, density\_eigenvalues\_positive};
\eqref{eq:obs-half}.
...
$|\Om|=|\Om|^{2}=1$, $H=0$ \tier{P} (the Schmidt decomposition enters as \tier{L}) \Lean{no\_separable\_parts, separable\_iff\_top\_eigenvalue\_one}. Hence before observation there are no separable subsystems carrying
...
\eqref{eq:certainty}, here as \LeanInline{eps0\_Mpcf\_pi}, and checked numerically; the equivalence itself is
this section's one input, the bridge between the gauge running and the tower's algebra. \tier{C}
\end{assumption}
...
$S=\ln N$. \emph{Proof:} \LeanWrap{witten\_entropy\_realized} (by \texttt{rfl}---the most
direct realisation), \LeanWrap{witten\_finite\_rank, witten\_two\_conjectures}. The entropy
...
flow of the static patch at the Gibbons--Hawking temperature (\LeanInline{tower\_scale\_group, tower\_flow\_fixes\_tau, omega\_flow\_group, omega\_flow\_invariant}).
...
classification. \emph{Proof:} \LeanWrap{omega\_flow\_invariant, tower\_scale\_group};
\cite{CLPW22}.
...
\LeanWrap{binary\_entropy\_half}; \cite{CLPW22}.
...
(Remark~\ref{rmk:residual}). \tier{P}
\Lean{lattice\_radius\_conjugate}\begin{equation}\label{eq:obs-identity}
...
\LeanFor{eq:obs-matter}{fisher\_unit\_iff, H\_half\_one\_bit}%
...
\tier{P} \Lean{energy\_per\_bit\_constant}
\end{proposition}
\begin{proof}
...
\end{equation}
\tier{P} \Lean{shell\_tension\_determined}
\end{proposition}
\begin{proof}
...
\tier{P} for the arithmetic; \tier{L} for the junction condition itself~\cite{Israel66}.
\Lean{israel\_prefactor, israel\_jump\_determined}
\end{proposition}
\begin{proof}
...
number of bits the level carries. \tier{P/N} \Lean{backreaction\_cumulative}
\end{remark}
...
$\golden^{-6}\cdot\golden^{6}=1$ (\LeanInline{G\_Lambda\_duality, R\_scalar\_pcf, BF\_value}).
...
flat---its curvature, computed, vanishes, $R^{(5)}=0$ (\LeanInline{ets\_constant\_block\_christoffels\_vanish})---and the
...
\emph{Proof:} \LeanWrap{dS\_temperature, dS\_beta\_reciprocal} (\S\ref{app:embedding});
...
(Proposition~\ref{prop:entropy-max}).
 \tier{P/N} \Lean{binary\_entropy\_half, S\_tower\_recurrence}
\end{proposition}
...
\Lean{ricciCoeff, ricciScalar, einsteinCoeff, R\_AB\_einstein, R\_scalar\_pcf, G\_AB\_pcf, BF\_value}
\tier{P}
\end{proposition}
...
local Rindler horizon yields the Einstein equation~\cite{Jacobson1995,Jacobson2016}.
 \tier{P} \Lean{dS\_einstein\_Lambda, R\_AB\_einstein, G\_AB\_pcf}
\end{theorem}
...
\Lean{tower\_first\_law, tower\_matter\_ingredients, S\_tower\_recurrence, binary\_entropy\_half}. The colour octet has $\dim\mathfrak{su}(3)=n^{2}-1=8$ at the arity
...
\LeanWrap{gauge\_dim\_su3, phi\_central\_chain, central\_chain, KK\_numerator, clifford\_S3\_condition, hopf\_from\_clifford, a2\_minimal\_vectors\_exactly\_six}.
The electroweak data follow from the same modulus: the weak mixing angle at unification is
$\sin^{2}\theta_W|_{\rm GUT}=\tfrac38=N(0)/N(2)$ \LeanWrap{weinberg\_angle\_gut}, running to $0.231$ at
$M_Z$; the tower entropy ratio $S(3)/S(6)=\golden^{-3}$ \LeanWrap{entropy\_ratio\_S3\_S6} marks the
breaking $\Z_3\to\Z_4$ at the tower midpoint $\sigma=3$, and the $G$--$\Lambda$ duality reads $\golden^{-6}\golden^{+6}=1$
\LeanWrap{G\_Lambda\_duality}. The generations are not placed radially by this $\sigma$-assignment:
...
\tier{P} (the vanishing-$00$ and spatial values are \tier{N}, symbolic) \Lean{dS\_first\_law, energy\_is\_inv\_H, komar\_first\_law, mu3\_temp\_ratio, LL\_spatial\_ne\_zero}
...
\LeanFor{eq:Lambda-from-curvature}{two\_routes\_to\_six}%
...
ladder rather than a continuum. \tier{N}
\LeanFor{eq:Lambda-from-curvature}{lambda\_magnitude\_eq\_level}%
\LeanFor{eq:interval-levels}{sigma\_G\_val, sigma\_EM\_val, sigma\_Lambda\_val, threshold\_ordering}%
\LeanFor{eq:interval-gap}{interval\_gap, interval\_gap\_general}%
\LeanFor{eq:interval-fractions}{em\_holographic\_fraction, em\_asymmetry}%
\LeanFor{eq:sigma-obs}{entropy\_at\_mu\_three, tower\_step\_ratio}%
\end{remark}
...
the angle is then evaluated on them. \tier{P}
\Lean{interval\_uniqueness, interval\_arity\_discriminates, interval\_gap\_only\_three}
\end{proposition}
\begin{proof}
...
\begin{figure}[htb]\centering
\includegraphics[width=\linewidth]{fig1_alphas_uniqueness.pdf}
\caption{(a) The spectral-angle surface $\sin\alpha(\sigma_1)\cos\alpha(\sigma_2)$ of
\eqref{eq:spectral-surface}, whose tangent climbs at the rate $\golden$ of
\eqref{eq:spectral-angle} of \S\ref{ssec:meta}; the marked point is the level pair
$(\sigma_G,\sigma_{\mathrm{EM}})=(2,3)$ of \eqref{eq:interval-levels}.
(b) How many of the four constraints of Proposition~\ref{prop:interval-uniqueness} each integer
triple $(\sigma_G,\sigma_{\mathrm{EM}},\sigma_\Lambda)$ satisfies: the boxed $(2,3,6)$ is the only
one satisfying all four, every other triple failing at least one. No fitted quantity enters either
panel.}
...
\end{description}
\LeanWrap{cw3\_observer\_items\_core}
...
\end{align*}
\LeanWrap{BH\_factor, BH\_factor\_modes, holographic\_area\_factor}
...
level, the Fibonacci quantum dimension (Corollary~\ref{cor:golden-growth}). \tier{N} (six checks);
D3 is \tier{P}
(\S\ref*{ssec:construction}, \LeanWrap{binary\_entropy\_half}).
\LeanWrap{Lambda5\_pcf, G\_Lambda\_duality}
\end{proposition}
\begin{proof}
...
Fig.~\ref{fig:tower-modes}. \tier{P/N}
\Lean{S\_tower, tower\_step\_ratio}
\end{corollary}
...
eigenvalues equal $\tfrac12=\lvert\Om\rvert$. \tier{P}
\Lean{projector\_idem, rho\_is\_state, rho\_k2\_eq\_half\_projector, projector\_trace\_eq\_rank}
\end{proposition}
...
lets the state reading and the amplitude reading be taken on one object. \tier{P}
\Lean{two\_positivities\_distinct, rho\_k2\_eq\_half\_projector}
\end{proposition}
\begin{proof}
...
(\S\ref{ssec:amp}). \tier{P}
\LeanFor{eq:frame-invariance}{projector\_frame\_invariant}%
\LeanFor{eq:four-faces}{projector\_idem, projector\_trace\_eq\_rank, rho\_is\_state, positroid\_from\_kp}%
\Lean{four\_faces\_one\_datum}
\end{theorem}
\begin{proof}
...
\tier{P} \Lean{eisenstein, Omega\_eigenvalues, su3\_dim}
\end{proposition}
\begin{proof}
...
combinatorial ladder is, rung by rung, the ladder of gauge groups (\S\ref{app:kk}). \tier{L}; the
dimension count is \tier{P}
\Lean{ladder\_from\_scott, scott\_kissing\_eq\_two\_posroots, kissing\_eq\_two\_posroots, dims\_even\_ladder, ladder\_dim\_eq\_kissing\_plus\_rank, fks\_ladder\_dims}
\end{proposition}
...
(\S\ref*{ssec:p-tower}, \eqref{eq:euler-product}). \tier{P}
\Lean{veneziano\_eq\_gamma\_ratio, regge\_tower\_is\_euler\_product}
\end{proposition}
...
canonical form (the $\golden$ side). \tier{P/N} \Lean{ft\_identity, ft\_limit, pentagon\_u\_eq\_golden}
\end{proposition}
...
\tier{P/N} \Lean{beta\_is\_mssm, sm\_anomaly\_free, weinberg\_angle\_gut}
\end{proposition}
...
tower's own clock.
\tier{P/N}; the double copy is \tier{L} (Bern--Carrasco--Johansson)
\Lean{fisher\_time\_lock, entropy\_response\_per\_bit, entropy\_response\_hasDerivAt}
\end{theorem}
\begin{proof}
...
\end{equation}
reproducing the MSSM one-loop coefficients with no superpartner particles. \tier{P}
\Lean{mssm\_eq\_sm\_plus\_bridges, bridge\_cocycle}
\end{theorem}
\begin{proof}
...
the decomposition of the identity that the groupoid of \S\ref{ssec:hinge} composes. \tier{P}
\Lean{er\_epr\_state\_eq\_geometry, er\_epr\_bridge}
\end{proposition}
...
independent: one sums representations, the other sums bridges. \tier{P}
\Lean{bridges\_reproduce\_mssm, beta\_is\_mssm}
\end{corollary}
...
the bridges of \S\ref{ssec:susy}. \tier{N} (Kodama--Williams enters as \tier{L}) \Lean{positroid\_from\_kp, bridge\_cocycle}
\end{proposition}
...
with the vacuum at $0$. \tier{P}
\Lean{towerHam\_isHermitian, casimirOp\_isHermitian, towerHam\_bounded\_below, casimirOp\_bounded\_below}
\end{proposition}
...
tower's own spacing, the other the colour energy. \tier{P}
\Lean{spectrum\_discrete, vacuum\_unique, mass\_gap, operators\_selfadjoint\_and\_isometry}
\end{proposition}
\begin{proof}
...
\tier{P} (the GKPW dictionary itself enters as \tier{L})
\LeanFor{eq:bulk-boundary-exp}{towerE\_eq\_exp\_regulator, S\_tower\_eq\_exp\_regulator}%
\LeanFor{eq:intertwine}{modular\_bulk\_ratio\_level\_independent, bulk\_boundary\_intertwine}%
\Lean{bulkBoundary\_isometry}
\end{theorem}
\begin{proof}
...
What is \emph{not} proved here is the GKPW dictionary itself; it enters cited, \tier{L}
...
$T=e^{-aH}$. \tier{P/N} \Lean{colour\_jacobi}
\end{proposition}
...
\begin{equation}\label{eq:gap}
  \Delta=\tfrac23 g^2>0. 
\end{equation}
\tier{P} \Lean{strong\_coupling\_gap}
\end{theorem}
\begin{proof}
...
arbitrary. \tier{P}
\Lean{theta\_pairing\_eq\_transfer, reflection\_positive\_general, half\_prop\_sq, theta\_involutive, theta\_preserves\_modulus\_C, theta\_commutes\_real\_diagonal, transfer\_positive, cpt\_involution, cpt\_preserves\_modulus}
\end{proposition}
\begin{proof}
...
\tier{P} \Lean{reflCov\_factors, reflQuadForm\_eq\_square, reflection\_positive\_measure}
\end{proposition}
\begin{proof}
...
Osterwalder--Schrader axioms. \tier{P}
\Lean{os\_reconstruction\_explicit, transfer\_spectral\_data, clustering\_exponential, vacuum\_transfer\_eq\_one, transfer\_gap\_below\_vacuum}
\end{theorem}
\begin{proof}
...
\tier{P} \Lean{os\_reconstruction\_explicit, towerHam\_isHermitian, towerHam\_bounded\_below}
\end{corollary}
...
(\S\ref*{ssec:p-tower}) it is the analyticity/temperedness axiom. \tier{P} \Lean{internal\_wick}
\end{remark}
...
gauge theory. \tier{P} \Lean{bulkBoundary\_isometry}
\end{theorem}
\begin{proof}
...
(\S\ref{ssec:susy}). \tier{P/N}
\Lean{regge\_tower\_is\_euler\_product, regge\_dirichlet\_eq\_zeta}
\end{theorem}
\begin{proof}
...
under the duality that relates strong and weak coupling. \tier{N}; the dual-superconductor mechanism
is \tier{L} ('t~Hooft--Mandelstam).
\Lean{colour\_gap\_pos, gap\_self\_dual\_invariant, self\_dual\_charges, bridge\_cocycle}
\end{proposition}
...
$V(\sigma)=\epsz\golden^{-\sigma}$\,\LeanWrap{swampland\_hasDerivAt}. The two are therefore conjugate,
...
fixed level by level rather than free. \tier{P} (the level-invariance $\sigma_{\mathrm{tension}}(\sigma)\,S(\sigma)=4\pi^4/(q^2\Mpcf)$ is proved: the $\golden^{\sigma}$ of the entropy cancels the $\golden^{-\sigma}$ of the condensate)
\LeanFor{eq:condensate-conjugate}{swampland\_hasDerivAt}%
\LeanFor{eq:tension-weld}{eps0\_Mpcf\_pi, tension\_welded\_to\_tower, tension\_weld\_value}%
\Lean{colour\_gap\_pos}
\end{proposition}
...
\tier{P} \Lean{S\_is\_operator\_spectrum, tension\_is\_conjugate\_spectrum, tension\_welded\_to\_tower, mass\_gap}
\end{proposition}
...
\tier{P} for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \tier{L} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \Lean{modular\_generator\_is\_entropy, modular\_generator\_is\_LL\_over\_T, modular\_generator\_is\_tower, scale\_flow\_is\_dilation, hubble\_descends\_tower}
\end{theorem}
...
\tier{P} \Lean{scale\_is\_modulus, one\_object}
\end{theorem}
...
Theorem~\ref{thm:zeta-odd}, and the character Proposition~\ref{prop:pentagon-chi5}. \tier{P}
\Lean{s\_duality\_exact, functional\_equation\_fixed\_at\_half, places\_assembly}
\end{theorem}
...
\Lean{Lambda\_QCD\_eq\_Lambda, gap\_independent\_of\_cutoff, Lambda\_QCD\_pos, eps0\_Mpcf\_pi}
\tier{P/N}
\end{corollary}
...
theorem (Theorem~\ref{thm:funct-eq}, Theorem~\ref{thm:duality}). \tier{P}
...
Proposition~\ref{prop:fluid}). \tier{P}
...
\item the theory sits at an ultraviolet fixed point, the asymptotic-safety corner of the duality web
      ...
      (\S\ref{ssec:accum})\,\LeanWrap{c\_tendsto\_two};
\item and that fixed point is not tuned: Assumption~\ref{asm:bridge} (\eqref{eq:obs-fixedpoint}) makes it \emph{equivalent} to the
      ...
      accumulated once around the fundamental domain\,\LeanWrap{eps0\_Mpcf\_pi}.
\end{itemize}
...
\Lean{phi\_central\_chain}---the same three that gives the colour arity $\lfloor\pi\rfloor$ and the
three eigenvalues $\tfrac12\omega^{k}$ of Proposition~\ref{prop:pcf-norms}. What the bridge supplies
is not the number but the resolution of the spin structures onto it. \tier{L} (bridge) \tier{P}
\Lean{framework\_mass\_gap, gap\_survives, eps0\_Mpcf\_pi}
\end{remark}
...
the section: each can be checked on its own without the surrounding reading. \tier{P}
\Lean{entropy\_dof\_core, gauge\_side\_facts, transfer\_spectral\_data}
\end{theorem}
...
\node[font=\bfseries,above=1mm] at (current bounding box.north)
  {\S\ref*{sec:dof} as one commutative diagram: seven movements A$\to$G, threaded by $\zeta$ (gold) and $\lvert\Om\rvert{=}\tfrac12$ (blue)};
