...
\Proof{projection\_closed\_form, M8\_epsilon0\_from\_projection}%
...
\Proof{omega_flow_invariant, bridge_cocycle, bridge_inverse, bridge_compose}
\end{proposition}
...
\Proof{bridge\_cocycle, bridge\_inverse, bridge\_compose}).
...
\Proof{cpt\_swaps\_P\_to\_F, cpt\_fixes\_C, cpt\_involution, cpt\_preserves\_modulus}.
...
\Proof{cw3\_observer\_items\_core}; \cite{CLPW22}.
...
$|\Om|=|\Om|^{2}=1$, $H=0$  (the Schmidt decomposition enters as \Hyp{}) . Hence before observation there are no separable subsystems carrying \Proof{no_separable_parts, separable_iff_top_eigenvalue_one}
...
$S=\ln N$. \emph{Proof:}
\Proof{witten\_entropy\_realized} (by \texttt{rfl}---the most direct realisation),
\Proof{witten\_finite\_rank, witten\_two\_conjectures}.
The entropy
...
\Proof{omega\_flow\_invariant, tower\_scale\_group} \cite{CLPW22}.
...
\Proof{binary\_entropy\_half} \cite{CLPW22}.
...
(Remark~\ref{rmk:residual}). \Proof{lattice_radius_conjugate}
...
\Proof{fisher\_unit\_iff, H\_half\_one\_bit}%
...
\end{equation}
\Proof{shell_tension_determined}
\end{proposition}
\begin{proof}
...
for the arithmetic; \Hyp{} for the junction condition itself~\cite{Israel66}. \Proof{israel_prefactor, israel_jump_determined}
...
number of bits the level carries. \Proof{backreaction_cumulative} \Numeric{eq:shell-tension}
\end{remark}
...
\Proof{binary_entropy_half, S_tower_recurrence} \Numeric{prop:entropy-max}
\end{proposition}
...
\Proof{G\_AB\_pcf, R\_AB\_einstein, R\_scalar\_pcf, einsteinCoeff, ricciCoeff, ricciScalar, BF\_value}
\end{proposition}
...
\Proof{dS_einstein_Lambda, R_AB_einstein, G_AB_pcf}
\end{theorem}
...
\Proof{gauge\_dim\_su3, phi\_central\_chain, central\_chain, KK\_numerator, clifford\_S3\_condition, hopf\_from\_clifford, a2\_minimal\_vectors\_exactly\_six}.
...
(the vanishing-$00$ and spatial values are \Numeric{eq:obs-landauer}, symbolic) \Proof{dS_first_law, energy_is_inv_H, komar_first_law, mu3_temp_ratio, LL_spatial_ne_zero}
...
\Proof{lambda\_magnitude\_eq\_level}%
\Proof{sigma\_G\_val, sigma\_EM\_val, sigma\_Lambda\_val, threshold\_ordering}%
\Proof{interval\_gap, interval\_gap\_general}%
\Proof{em\_holographic\_fraction, em\_asymmetry}%
\Proof{entropy\_at\_mu\_three, tower\_step\_ratio}%
\end{remark}
...
the angle is then evaluated on them. \Proof{interval_uniqueness, interval_arity_discriminates, interval_gap_only_three}
...
D3 is \Proof{binary_entropy_half}
...
Fig.~\ref{fig:tower-modes}. \Proof{S_tower, tower_step_ratio}
...
eigenvalues equal $\tfrac12=\lvert\Om\rvert$. \Proof{projector_idem, rho_is_state, rho_k2_eq_half_projector, projector_trace_eq_rank}
...
lets the state reading and the amplitude reading be taken on one object. \Proof{two_positivities_distinct, rho_k2_eq_half_projector}
...
(\S\ref{ssec:amp}). \Proof{projector_frame_invariant}
%
\Proof{projector\_idem, projector\_trace\_eq\_rank, rho\_is\_state, positroid\_from\_kp}%
\end{theorem}
\begin{proof}
...
\Proof{eisenstein, Omega_eigenvalues, su3_dim}
\end{proposition}
\begin{proof}
...
dimension count is \Proof{ladder_from_scott, scott_kissing_eq_two_posroots, kissing_eq_two_posroots, dims_even_ladder, ladder_dim_eq_kissing_plus_rank, fks_ladder_dims}
...
(\S\ref*{ssec:p-tower}, \eqref{eq:euler-product}). \Proof{veneziano_eq_gamma_ratio, regge_tower_is_euler_product}
...
canonical form (the $\golden$ side). \Proof{ft_identity, ft_limit, pentagon_u_eq_golden}
\end{proposition}
...
\Proof{beta_is_mssm, sm_anomaly_free, weinberg_angle_gut}
\end{proposition}
...
tower's own clock.
; the double copy is \Hyp{} (Bern--Carrasco--Johansson) \Proof{fisher_time_lock, entropy_response_per_bit, entropy_response_hasDerivAt}
...
\end{equation}
reproducing the MSSM one-loop coefficients with no superpartner particles. \Proof{mssm_eq_sm_plus_bridges, bridge_cocycle}
...
the decomposition of the identity that the groupoid of \S\ref{ssec:hinge} composes. \Proof{er_epr_state_eq_geometry, er_epr_bridge}
...
independent: one sums representations, the other sums bridges. \Proof{bridges_reproduce_mssm, beta_is_mssm}
...
with the vacuum at $0$. \Proof{towerHam_isHermitian, casimirOp_isHermitian, towerHam_bounded_below, casimirOp_bounded_below}
...
tower's own spacing, the other the colour energy. \Proof{spectrum_discrete, vacuum_unique, mass_gap, operators_selfadjoint_and_isometry}
...
(the GKPW dictionary itself enters as \Hyp{}) \Proof{towerE_eq_exp_regulator, S_tower_eq_exp_regulator}
%
\Proof{modular\_bulk\_ratio\_level\_independent, bulk\_boundary\_intertwine}%
\end{theorem}
\begin{proof}
...
arbitrary. \Proof{theta_pairing_eq_transfer, reflection_positive_general, half_prop_sq, theta_involutive, theta_preserves_modulus_C, theta_commutes_real_diagonal, transfer_positive, cpt_involution, cpt_preserves_modulus}
...
\Proof{reflCov_factors, reflQuadForm_eq_square, reflection_positive_measure}
\end{proposition}
\begin{proof}
...
Osterwalder--Schrader axioms. \Proof{os_reconstruction_explicit, transfer_spectral_data, clustering_exponential, vacuum_transfer_eq_one, transfer_gap_below_vacuum}
...
\Proof{os_reconstruction_explicit, towerHam_isHermitian, towerHam_bounded_below}
\end{corollary}
...
(\S\ref*{ssec:p-tower}) it is the analyticity/temperedness axiom. \Proof{internal_wick}
\end{remark}
...
gauge theory. \Proof{bulkBoundary_isometry}
\end{theorem}
\begin{proof}
...
(\S\ref{ssec:susy}). \Proof{regge_tower_is_euler_product, regge_dirichlet_eq_zeta} \Numeric{prop:veneziano}
...
fixed level by level rather than free.  (the level-invariance $\sigma_{\mathrm{tension}}(\sigma)\,S(\sigma)=4\pi^4/(q^2\Mpcf)$ is proved: the $\golden^{\sigma}$ of the entropy cancels the $\golden^{-\sigma}$ of the condensate) \Proof{swampland_hasDerivAt}
%
\Proof{eps0\_Mpcf\_pi, tension\_welded\_to\_tower, tension\_weld\_value}%
\end{proposition}
...
\Proof{S_is_operator_spectrum, tension_is_conjugate_spectrum, tension_welded_to_tower, mass_gap}
\end{proposition}
...
for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \Hyp{} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \Proof{modular_generator_is_entropy, modular_generator_is_LL_over_T, modular_generator_is_tower, scale_flow_is_dilation, hubble_descends_tower}
\end{theorem}
...
\Proof{M_eq_Mpcf, colour_scale_from_M, colour_running_fixed}
\end{theorem}
...
\Proof{scale_is_modulus, one_object}
\end{theorem}
...
Theorem~\ref{thm:zeta-odd}, and the character Proposition~\ref{prop:pentagon-chi5}. \Proof{s_duality_exact, functional_equation_fixed_at_half, places_assembly}
...
\Proof{gap\_independent\_of\_cutoff, eps0\_Mpcf\_pi, Lambda\_QCD\_eq\_Lambda, Lambda\_QCD\_pos} \Numeric{eq:transmutation}
\end{corollary}
...
theorem (Theorem~\ref{thm:funct-eq}, Theorem~\ref{thm:duality}). \Proof{fixed_line_is_facePhi, functional_equation_fixed_line, places_assembly, s_duality_exact, s_duality_fixed_point_unique, theta_from_jacobi, functional_equation_fixed_at_half, places_assembly, s_duality_exact}
...
Proposition~\ref{prop:fluid}). \Proof{er_epr_bridge, er_epr_state_eq_geometry, bridge_cocycle, positroid_from_kp}
...
is not the number but the resolution of the spin structures onto it. \Hyp{} (bridge) \Proof{framework_mass_gap, gap_survives, eps0_Mpcf_pi}
...
the section: each can be checked on its own without the surrounding reading. \Proof{entropy_dof_core, gauge_side_facts, transfer_spectral_data}
