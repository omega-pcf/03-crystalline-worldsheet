...
$|\Om|=|\Om|^{2}=1$, $H=0$  (the Schmidt decomposition enters as \HypWitness{}) . Hence before observation there are no separable subsystems carrying \ProofWitness{no\_separable\_parts, separable\_iff\_top\_eigenvalue\_one}
...
for the arithmetic; \HypWitness{} for the junction condition itself~\cite{Israel66}. \ProofWitness{israel\_prefactor, israel\_jump\_determined}
...
combinatorial ladder is, rung by rung, the ladder of gauge groups (\S\ref{app:kk}). ; the \HypWitness{}
...
tower's own clock.
; the double copy is \HypWitness{} (Bern--Carrasco--Johansson) \ProofWitness{fisher\_time\_lock, entropy\_response\_per\_bit, entropy\_response\_hasDerivAt}
...
the bridges of \S\ref{ssec:susy}.  (Kodama--Williams enters as \HypWitness{}) \ProofWitness{positroid\_from\_kp, bridge\_cocycle} \NumericalWitness{TODO}
\end{proposition}
...
(the GKPW dictionary itself enters as \HypWitness{}) \ProofWitness{towerE\_eq\_exp\_regulator, S\_tower\_eq\_exp\_regulator}
%
...
What is \emph{not} proved here is the GKPW dictionary itself; it enters cited, \HypWitness{}
...
is  ('t~Hooft--Mandelstam). \HypWitness{}
\end{proposition}
...
for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \HypWitness{} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \ProofWitness{modular\_generator\_is\_entropy, modular\_generator\_is\_LL\_over\_T, modular\_generator\_is\_tower, scale\_flow\_is\_dilation, hubble\_descends\_tower}
\end{theorem}
...
(data comparison only) \ProofWitness{M\_eq\_Mpcf} \HypWitness{}
\end{remark}
