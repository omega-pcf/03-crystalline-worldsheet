...
\ProofWitnessBlock{projection\_closed\_form, M8\_epsilon0\_from\_projection}%
...
$|\Om|_\sigma=\tfrac12$. \emph{Proof:} \ProofWitness{eisenstein\_cube, omega\_conj}; \cite{Witten01dS}.
...
(\ProofWitness{M2\_gamma\_half}), computed where it is stated. The particle is then a type~I
...
(\S\ref{app:gauge}; \ProofWitness{gauge\_dim\_su3, hopf\_from\_clifford, weinberg\_angle\_gut}).
...
\ProofWitnessBlock{cw3\_observer\_items\_core}; \cite{CLPW22}.
...
integer. \emph{Proof:} \ProofWitness{witten\_finite\_rank}; \eqref{eq:obs-spinstar}.
...
$|\Om|=|\Om|^{2}=1$, $H=0$  (the Schmidt decomposition enters as \HypWitness{\cite{Schmidt1907}}) . Hence before observation there are no separable subsystems carrying \ProofWitness{no\_separable\_parts, separable\_iff\_top\_eigenvalue\_one}
...
\eqref{eq:certainty}, here as \ProofWitness{eps0\_Mpcf\_pi}, and checked numerically; the equivalence itself is
...
flow of the static patch at the Gibbons--Hawking temperature (\ProofWitness{tower\_scale\_group, tower\_flow\_fixes\_tau, omega\_flow\_group, omega\_flow\_invariant}).
...
\ProofWitnessBlock{omega\_flow\_invariant, tower\_scale\_group} \cite{CLPW22}.
...
\ProofWitnessBlock{binary\_entropy\_half} \cite{CLPW22}.
...
(Remark~\ref{rmk:residual}). \ProofWitness{lattice\_radius\_conjugate}
...
for the arithmetic; \HypWitness{\cite{Israel66}} for the junction condition itself~\cite{Israel66}. \ProofWitness{israel\_prefactor, israel\_jump\_determined}
...
number of bits the level carries. \ProofWitness{backreaction\_cumulative} \NumericalWitness{eq:shell-tension}
\end{remark}
...
\emph{Proof:} \ProofWitness{dS\_temperature, dS\_beta\_reciprocal} (\S\ref{app:embedding}) \cite{GibbonsHawking77}.
...
\ProofWitnessBlock{binary\_entropy\_half, S\_tower\_recurrence} \NumericalWitness{prop:entropy-max}
\end{proposition}
...
\ProofWitnessBlock{G\_AB\_pcf, R\_AB\_einstein, R\_scalar\_pcf, einsteinCoeff, ricciCoeff, ricciScalar, BF\_value}
\end{proposition}
...
(the vanishing-$00$ and spatial values are \NumericalWitness{eq:obs-landauer}, symbolic) \ProofWitness{dS\_first\_law, energy\_is\_inv\_H, komar\_first\_law, mu3\_temp\_ratio, LL\_spatial\_ne\_zero}
...
ladder rather than a continuum. \NumericalWitness{eq:Lambda-from-curvature}
...
the angle is then evaluated on them. \ProofWitness{interval\_uniqueness, interval\_arity\_discriminates, interval\_gap\_only\_three}
...
level, the Fibonacci quantum dimension (Corollary~\ref{cor:golden-growth}).  (six checks); \NumericalWitness{cor:golden-growth}
D3 is \ProofWitness{binary\_entropy\_half}
...
Fig.~\ref{fig:tower-modes}. \ProofWitness{S\_tower, tower\_step\_ratio}
...
eigenvalues equal $\tfrac12=\lvert\Om\rvert$. \ProofWitness{projector\_idem, rho\_is\_state, rho\_k2\_eq\_half\_projector, projector\_trace\_eq\_rank}
...
lets the state reading and the amplitude reading be taken on one object. \ProofWitness{two\_positivities\_distinct, rho\_k2\_eq\_half\_projector}
...
combinatorial ladder is, rung by rung, the ladder of gauge groups (\S\ref{app:kk}). ; the \HypWitness{\cite{Witten95}}
dimension count is \ProofWitness{ladder\_from\_scott, scott\_kissing\_eq\_two\_posroots, kissing\_eq\_two\_posroots, dims\_even\_ladder, ladder\_dim\_eq\_kissing\_plus\_rank, fks\_ladder\_dims}
...
(\S\ref*{ssec:p-tower}, \eqref{eq:euler-product}). \ProofWitness{veneziano\_eq\_gamma\_ratio, regge\_tower\_is\_euler\_product}
...
canonical form (the $\golden$ side). \ProofWitness{ft\_identity, ft\_limit, pentagon\_u\_eq\_golden}
\end{proposition}
...
tower's own clock.
; the double copy is \HypWitness{\cite{BCJ}} (Bern--Carrasco--Johansson) \ProofWitness{fisher\_time\_lock, entropy\_response\_per\_bit, entropy\_response\_hasDerivAt}
...
\end{equation}
reproducing the MSSM one-loop coefficients with no superpartner particles. \ProofWitness{mssm\_eq\_sm\_plus\_bridges, bridge\_cocycle}
...
the decomposition of the identity that the groupoid of \S\ref{ssec:hinge} composes. \ProofWitness{er\_epr\_state\_eq\_geometry, er\_epr\_bridge}
...
independent: one sums representations, the other sums bridges. \ProofWitness{bridges\_reproduce\_mssm, beta\_is\_mssm}
...
the bridges of \S\ref{ssec:susy}.  (Kodama--Williams enters as \HypWitness{\cite{KodamaWilliams}}) \ProofWitness{positroid\_from\_kp, bridge\_cocycle} \NumericalWitness{TODO}
\end{proposition}
...
with the vacuum at $0$. \ProofWitness{towerHam\_isHermitian, casimirOp\_isHermitian, towerHam\_bounded\_below, casimirOp\_bounded\_below}
...
tower's own spacing, the other the colour energy. \ProofWitness{spectrum\_discrete, vacuum\_unique, mass\_gap, operators\_selfadjoint\_and\_isometry}
...
(the GKPW dictionary itself enters as \HypWitness{\cite{GKP98,Witten98}}) \ProofWitness{towerE\_eq\_exp\_regulator, S\_tower\_eq\_exp\_regulator}
%
\ProofWitnessBlock{modular\_bulk\_ratio\_level\_independent, bulk\_boundary\_intertwine}%
\end{theorem}
\begin{proof}
...
What is \emph{not} proved here is the GKPW dictionary itself; it enters cited, \HypWitness{\cite{GKP98,Witten98}}
...
$T=e^{-aH}$. \ProofWitness{colour\_jacobi}
\end{proposition}
...
arbitrary. \ProofWitness{theta\_pairing\_eq\_transfer, reflection\_positive\_general, half\_prop\_sq, theta\_involutive, theta\_preserves\_modulus\_C, theta\_commutes\_real\_diagonal, transfer\_positive, cpt\_involution, cpt\_preserves\_modulus}
...
\ProofWitnessBlock{reflCov\_factors, reflQuadForm\_eq\_square, reflection\_positive\_measure}
\end{proposition}
\begin{proof}
...
Osterwalder--Schrader axioms. \ProofWitness{os\_reconstruction\_explicit, transfer\_spectral\_data, clustering\_exponential, vacuum\_transfer\_eq\_one, transfer\_gap\_below\_vacuum}
...
\ProofWitnessBlock{os\_reconstruction\_explicit, towerHam\_isHermitian, towerHam\_bounded\_below}
\end{corollary}
...
(\S\ref*{ssec:p-tower}) it is the analyticity/temperedness axiom. \ProofWitness{internal\_wick}
\end{remark}
...
gauge theory. \ProofWitness{bulkBoundary\_isometry}
\end{theorem}
\begin{proof}
...
(\S\ref{ssec:susy}). \ProofWitness{regge\_tower\_is\_euler\_product, regge\_dirichlet\_eq\_zeta} \NumericalWitness{prop:veneziano}
...
is \HypWitness{\cite{KogutSusskind}} ('t~Hooft--Mandelstam).
\end{proposition}
...
$V(\sigma)=\epsz\golden^{-\sigma}$\,\ProofWitness{swampland\_hasDerivAt}
...
fixed level by level rather than free.  (the level-invariance $\sigma_{\mathrm{tension}}(\sigma)\,S(\sigma)=4\pi^4/(q^2\Mpcf)$ is proved: the $\golden^{\sigma}$ of the entropy cancels the $\golden^{-\sigma}$ of the condensate) \ProofWitness{swampland\_hasDerivAt}
%
\ProofWitnessBlock{eps0\_Mpcf\_pi, tension\_welded\_to\_tower, tension\_weld\_value}%
\end{proposition}
...
for the proved core---the modular flow is the tower's scale group, $\hat K_{\mathrm{mod}}=S(\sigma)$, and the descent $H(\sigma)\propto\golden^{-\sigma/2}$; \HypWitness{\cite{BisognanoWichmann75}} for the Bisognano--Wichmann/KMS identification and the CLPW crossed product, which enter as hypotheses. \ProofWitness{modular\_generator\_is\_entropy, modular\_generator\_is\_LL\_over\_T, modular\_generator\_is\_tower, scale\_flow\_is\_dilation, hubble\_descends\_tower}
\end{theorem}
...
(data comparison only) \ProofWitness{M\_eq\_Mpcf}
\end{remark}
...
Theorem~\ref{thm:zeta-odd}, and the character Proposition~\ref{prop:pentagon-chi5}. \ProofWitness{s\_duality\_exact, functional\_equation\_fixed\_at\_half, places\_assembly}
...
\ProofWitnessBlock{gap\_independent\_of\_cutoff, eps0\_Mpcf\_pi, Lambda\_QCD\_eq\_Lambda, Lambda\_QCD\_pos} \NumericalWitness{eq:transmutation}
\end{corollary}
...
any holographic model. \ProofWitness{strong\_coupling\_gap}
...
theorem (Theorem~\ref{thm:funct-eq}, Theorem~\ref{thm:duality}). \ProofWitness{fixed\_line\_is\_facePhi, functional\_equation\_fixed\_line, places\_assembly, s\_duality\_exact, s\_duality\_fixed\_point\_unique, theta\_from\_jacobi, functional\_equation\_fixed\_at\_half, places\_assembly, s\_duality\_exact}
...
Proposition~\ref{prop:fluid}). \ProofWitness{er\_epr\_bridge, er\_epr\_state\_eq\_geometry, bridge\_cocycle, positroid\_from\_kp}
...
is not the number but the resolution of the spin structures onto it. \HypWitness{} (bridge) \ProofWitness{framework\_mass\_gap, gap\_survives, eps0\_Mpcf\_pi}
...
the section: each can be checked on its own without the surrounding reading. \ProofWitness{entropy\_dof\_core, gauge\_side\_facts, transfer\_spectral\_data}
