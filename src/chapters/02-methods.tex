...
only if $x=\tfrac12$. \ProofWitnessBlock{faceInv\_apex}
\end{proposition}
...
\ProofWitnessBlock{faceGalois\_apex}
\end{proposition}
...
depends directly on itself). \ProofWitnessBlock{DistributedSelfReference}
\end{definition}
...
\ProofWitnessBlock{M6\_phi\_root\_unique, M6\_characteristic\_root\_is\_phi} \NumericalWitness{eq:phisq}
\end{theorem}
...
\ProofWitnessBlock{normP\_eq\_tan, normF\_eq\_cos, Omega\_eigenvalues}
\end{proposition}
\begin{proof}
...
$V^{\dagger}V=\mathbf1$. \ProofWitnessBlock{bulkBoundary\_isometry}
\end{lemma}
\begin{proof}
...
\ProofWitnessBlock{spectral\_uniqueness, bulkBoundary\_isometry}
\end{remark}
...
of the generator of \eqref{eq:base}. \ProofWitnessBlock{M\_PCF\_eq\_pi\_div\_eps0, M8\_epsilon0\_from\_projection, bulkBoundary\_isometry}
\end{proposition}
\begin{proof}
...
the general congruence for every $q\neq2,5$ is Lucas' and enters as \HypWitness{\cite{Lucas1878}}
\end{proposition}
...
\end{equation}
by \eqref{eq:binet}. \ProofWitnessBlock{psiGolden, psi\_on\_powers, psi\_golden\_fib}
\end{definition}
...
(Theorem~\ref{thm:basel}). \ProofWitnessBlock{even\_zeta\_bernoulli} \NumericalWitnessBlock{thm:basel}
\end{theorem}
\begin{proof}
...
here rather than assumed. \ProofWitnessBlock{idealNormK, idealEulerK, euler\_from\_ideals\_K}
...
\ProofWitnessBlock{degree\_formula\_K, ideal\_norm\_values\_K, euler\_from\_ideals\_K, vonMangoldtK\_values, local\_dedekind\_ramified\_K, local\_dedekind\_split\_K, local\_dedekind\_inert\_K}
\end{proposition}
...
$\Re s>1$ enters as \HypWitness{\cite{Riemann1859}}
\end{proposition}
...
(Poisson summation enters as \HypWitness{Poisson summation}) \ProofWitness{gammaR\_from\_tate}
%
...
evaluations enter as ) \HypWitness{\cite{Jacobi1829}}
\end{remark}
...
type~\cite{ConnesConsaniSite}. Proposition~\ref{prop:archimedean} supplies it. \ProofWitness{chi5\_even}
\end{remark}
...
leg.
\ProofWitnessBlock{functional\_equation\_fixed\_line, fixed\_line\_is\_facePhi, places\_assembly}
\end{theorem}
\begin{proof}
...
\begin{equation}\label{eq:hK}
  h_K=1 .
\end{equation}
(Minkowski's theorem enters as \HypWitness{\cite{Minkowski1896}}) \ProofWitness{minkowski\_bound\_lt\_two}
%
...
proved; the formula itself is the input.  (the class-number formula enters as \HypWitness{analytic class-number formula}) \ProofWitness{L1\_from\_class\_number\_formula, L1\_eq\_period\_over\_pi\_sqrt5}
%
\end{theorem}
...
entropy happens to equal one bit: it is the point where it is \emph{largest}. \ProofWitness{entropy\_slack\_nonneg, binary\_entropy\_le\_one, binary\_entropy\_half}
...
rather than deferred.  (the global Dedekind factorisation enters as \HypWitness{global Dedekind factorisation}) \ProofWitness{odd\_zeta\_ratio}
...
$\Re s=\mu$ (Theorem~\ref{thm:funct-eq}). \ProofWitness{section2\_master, places\_assembly}
\ProofWitnessBlock{fixed\_line\_is\_facePhi, functional\_equation\_fixed\_line, places\_assembly, s\_duality\_exact, s\_duality\_fixed\_point\_unique, theta\_from\_jacobi}
\end{theorem}
...
\ProofWitnessBlock{spacingScale, scale\_injective\_in\_conductor}
\end{proposition}
...
\ProofWitnessBlock{envelope, envelope\_splits, field\_part\_distinguishes, CountingLaw, DerivedUnit, the\_unit\_is\_derived}
\end{proposition}
...
\ProofWitnessBlock{sdualMate, sdual\_conj\_commute, sdualMate\_involutive}
\end{definition}
...
\ProofWitnessBlock{sdualMate\_im, sdualMate\_re, sdualMate\_fixed\_iff}
\end{proposition}
...
\ProofWitnessBlock{repulsion\_gives\_the\_modulus} \NumericalWitness{prop:repulsion-modulus}
\end{proposition}
...
the Polyakov action \eqref{eq:polyakov} (an \HypWitness{\cite{Polyakov1987}} input);
