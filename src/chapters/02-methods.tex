...
\Proof{phi_trace, phi_norm, galois_pair, phi_discriminant}
\end{definition}
...
\Proof{galois_conj_is_one_sub, galois_involution_is_one_sub}
\end{lemma}
...
\Proof{phi_lt_two, two_lt_phi_sq, two_not_in_golden_lattice, anchor_is_exterior}
\end{proposition}
...
$\golden=2\cos(\pi/5)$. \Proof{phi_eq_two_cos_pi_fifth}
\end{theorem}
\medskip
\begin{proof}
...
\Proof{M2_gamma_half, gaussian_integral_value, gaussian_weight_phi}
\end{lemma}
\medskip
...
\end{equation}
\Proof{gauss_normalised_iff, fourier_amplitude_iff, fourier_width_iff, selfdual_gaussian_unique}
\end{proposition}
\begin{proof}
...
only if $x=\tfrac12$. \Proof{faceInv_apex}
\end{proposition}
...
\Proof{M6_phi_root_unique, M6_characteristic_root_is_phi} \Numeric{eq:phisq}
\end{theorem}
...
of $\Rpcf$ and not of $\OK$. \Proof{phi_is_algebraic_integer, half_not_algebraic_integer}
...
should have the invariant named before it is used. \Proof{regulator_K_pos, period_eq_two_pi_regulator}
...
\Proof{normP_eq_tan, normF_eq_cos, Omega_eigenvalues}
\end{proposition}
\begin{proof}
...
\Proof{triad_re, triad_trace_zero, triad_two_products, triad_det_is_phi_pow}
\end{proposition}
...
\Proof{spectral_uniqueness, bulkBoundary_isometry}
\end{remark}
...
\end{equation}
\Proof{M8_epsilon0_from_projection}
\end{theorem}
\begin{proof}
...
\Proof{fibre\_independent\_of\_base, fibre\_monodromy, fibre\_reflection\_order\_two}
\end{proposition}
\begin{proof}
...
of the generator of \eqref{eq:base}. \Proof{M_PCF_eq_pi_div_eps0}
%
\Proof{M8\_epsilon0\_from\_projection, bulkBoundary\_isometry}%
\end{proposition}
\begin{proof}
...
\Proof{farishRot\_cube, farishRot\_diag, farishRot\_preserves\_dot, farishRot\_preserves\_norm,
...
\end{equation}
\Proof{M9\_collapse, M9\_eq\_half}
\end{theorem}
...
\end{equation}
\Proof{M10\_sigma\_from\_basel, M10\_sin\_cos\_mu}
\end{proposition}
...
\Proof{M11\_factorial\_face, M11\_factorial\_six}
\end{proposition}
...
\end{equation}
multiplicative on $(\Z/5\Z)^{\times}$, with $\sum_{a=0}^{4}\chi_5(a)=0$. \Proof{chi5, chi5_even, chi5_mul_on_units, chi5_sum_zero}
...
\Proof{norm_cosine_phi, phi_eq_two_cos_pi_fifth}
\end{proposition}
\begin{proof}
...
read in \eqref{eq:chi5-pentagon}, and $\golden$ is the value in both. \Proof{sin_ratio_eq_two_cos, pentagon_log_signature}
...
$\OK$.  for the thirteen primes decided in Lean and the twenty-three verified numerically; \Proof{mersenne_bridge, phi_pow_fib}
...
Proposition~\ref{prop:fib-criterion} exploits. \Proof{binet_general, phi_pow_fib}
\end{definition}
...
\end{equation}
by \eqref{eq:binet}. \Proof{psiGolden, psi_on_powers, psi_golden_fib}
\end{definition}
...
(Theorem~\ref{thm:basel}). \Proof{even_zeta_bernoulli} \Numeric{thm:basel}
\end{theorem}
\begin{proof}
...
\end{equation}
by unique factorisation in $\Z$. \Proof{regge_dirichlet_eq_zeta, regge_tower_is_euler_product}
...
here rather than assumed. \Proof{idealNormK, idealEulerK, euler_from_ideals_K}
...
\Proof{degree_formula_K, ideal_norm_values_K}
%
\Proof{euler\_from\_ideals\_K}%
\Proof{vonMangoldtK\_values}%
\Proof{local\_dedekind\_ramified\_K, local\_dedekind\_split\_K, local\_dedekind\_inert\_K}%
\end{proposition}
...
(Poisson summation enters as \Hyp{}) \Proof{gammaR_from_tate}
%
\Proof{theta\_from\_jacobi, theta\_fixed\_point\_unique}%
\end{proposition}
\begin{proof}
...
\end{equation}
with $\golden^{\mu\lng}=\sqrt2$ of Proposition~\ref{prop:mediation}.  (the two classical \Proof{M4_sqrt2, M4_sqrt3}
...
\Proof{framework_partition, framework_partition_eq_completed_zeta, places_assembly}
\end{theorem}
\begin{proof}
...
type~\cite{ConnesConsaniSite}. Proposition~\ref{prop:archimedean} supplies it. \Proof{chi5_even}
\end{remark}
...
\begin{equation}\label{eq:hK}
  h_K=1 .
\end{equation}
(Minkowski's theorem enters as \Hyp{}) \Proof{minkowski_bound_lt_two}
%
\Proof{class\_number\_one\_K}%
\end{proposition}
...
proved; the formula itself is the input.  (the class-number formula enters as \Hyp{}) \Proof{L1_from_class_number_formula, L1_eq_period_over_pi_sqrt5}
%
\end{theorem}
...
it is the regulator of $\Q(\sqrt5)$, measured in the unit $\lambda_{\log}$. \Proof{lambda_log_mul_regulator, sqrt5_half_L1_eq_regulator, entropy_bridge}
...
entropy happens to equal one bit: it is the point where it is \emph{largest}. \Proof{entropy_slack_nonneg, binary_entropy_le_one, binary_entropy_half}
...
rather than deferred.  (the global Dedekind factorisation enters as \Hyp{}) \Proof{odd_zeta_ratio}
...
value.
\Proof{mu\_diagram\_commutes, mu\_faces\_pairwise\_eq}
\end{theorem}
...
\Proof{M11_factorial_six, s3_orders, sigma_geom_from_S3}
\end{lemma}
...
\Proof{sigma\_diagram\_commutes}
\end{theorem}
...
\Proof{fixed_line_is_facePhi, functional_equation_fixed_line, places_assembly, s_duality_exact, s_duality_fixed_point_unique, theta_from_jacobi}
\end{theorem}
...
\Proof{two_sq_eq_neg_one_mod_five, five_splits_gaussian, conductor_four_is_order_of_turn, binary_period_lives_in_the_five_component, four_cocone}
\end{proposition}
...
\Proof{conductor_splits, fiber_mod4, fiber_mod5, chi5_is_the_mod5_fiber, mod4_does_not_determine_chi5, mod5_does_not_determine_chi4, conductor_attribution}
\end{proposition}
...
\Proof{spacingScale, scale_injective_in_conductor}
\end{proposition}
...
\Proof{envelope, envelope_splits, field_part_distinguishes, CountingLaw, DerivedUnit, the_unit_is_derived}
\end{proposition}
...
\Proof{sdualMate, sdual_conj_commute, sdualMate_involutive}
\end{definition}
...
\Proof{sdualMate_im, sdualMate_re, sdualMate_fixed_iff}
\end{proposition}
...
\Proof{repulsion_excludes_zero_spacing, repulsion_excludes_the_extreme_case}
\end{proposition}
...
\Proof{E_iff_no_shared_ordinate}
\end{theorem}
...
\Proof{repulsion_gives_the_modulus}
\end{proposition}
...
\Proof{tower_spacing_is_the_regulator}
\end{remark}
