...
% ---------------------------------------------------------------------
\noindent\emph{Verification tags:} $\ProofWitness{...}$ formal proof (Lean), $\NumericalWitness{...}$ numerical check,
$\HypWitness{...}$ hypothesis from the literature, $\Conj$ proposed. Compound tags
($\ProofWitness{...}\;\NumericalWitness{...}$) mark a claim resting on more than one, and each input is named where it enters.
...
\ProofWitnessBlock{phi\_trace, phi\_norm, galois\_pair, phi\_discriminant}
\end{definition}
...
\ProofWitnessBlock{galois\_conj\_is\_one\_sub, galois\_involution\_is\_one\_sub}
\end{lemma}
...
the present section consumes. \ProofWitnessBlock{mersenne\_bridge}
...
\ProofWitnessBlock{phi\_lt\_two, two\_lt\_phi\_sq, two\_not\_in\_golden\_lattice, anchor\_is\_exterior}
\end{proposition}
...
$\golden=2\cos(\pi/5)$. \ProofWitnessBlock{phi\_eq\_two\_cos\_pi\_fifth}
\end{theorem}
\medskip
\begin{proof}
...
\end{equation}
\ProofWitnessBlock{pi\_eq\_five\_arccos}
\end{proposition}
...
\end{equation}
\ProofWitnessBlock{gamma\_integral}
\end{proposition}
...
\ProofWitnessBlock{gamma\_half, gaussian\_integral\_value, gaussian\_weight\_phi}
\end{lemma}
\medskip
...
\end{equation}
\ProofWitnessBlock{half\_factorial}
\end{theorem}
...
\end{equation}
\ProofWitnessBlock{gauss\_normalised\_iff, fourier\_amplitude\_iff, fourier\_width\_iff, selfdual\_gaussian\_unique}
\end{proposition}
\begin{proof}
...
\end{equation}
\ProofWitnessBlock{sqrt2\_mediation, sqrt3\_mediation}
\end{proposition}
...
only if $x=\tfrac12$. \ProofWitnessBlock{face\_inv\_apex}
\end{proposition}
...
\ProofWitnessBlock{face\_galois\_apex}
\end{proposition}
...
depends directly on itself). \ProofWitnessBlock{distributed\_self\_reference}
\end{definition}
...
\ProofWitnessBlock{phi\_root\_unique, characteristic\_root\_is\_phi}
\NumericalWitnessBlock{eq:phisq}
\end{theorem}
...
\ProofWitnessBlock{spectral\_uniqueness}
\end{proposition}
...
of $\Rpcf$ and not of $\OK$. \ProofWitnessBlock{phi\_is\_algebraic\_integer, half\_not\_algebraic\_integer}
...
should have the invariant named before it is used. \ProofWitnessBlock{regulator\_K\_pos, period\_eq\_two\_pi\_regulator}
...
\ProofWitnessBlock{norm\_p\_eq\_tan, norm\_f\_eq\_cos, omega\_eigenvalues}
\end{proposition}
\begin{proof}
...
\ProofWitnessBlock{triad\_re, triad\_trace\_zero, triad\_two\_products, triad\_det\_is\_phi\_pow}
\end{proposition}
...
$V^{\dagger}V=\mathbf1$. \ProofWitnessBlock{bulk\_boundary\_isometry}
\end{lemma}
\begin{proof}
...
\ProofWitnessBlock{spectral\_uniqueness, bulk\_boundary\_isometry}
\end{remark}
...
\end{equation}
\ProofWitnessBlock{projection\_PCF}
\end{definition}
...
\eqref{eq:norms-derived}. \ProofWitnessBlock{oplus\_formula}
\end{remark}
...
\end{equation}
\ProofWitnessBlock{epsilon0\_from\_projection}
\end{theorem}
\begin{proof}
...
\ProofWitnessBlock{fibre\_independent\_of\_base, fibre\_monodromy, fibre\_reflection\_order\_two}
\end{proposition}
\begin{proof}
The fibre monodromy is $e^{i\pi}=-1$, of order two, $(-1)^2=1$, a $\Z/2$ datum that holds with
no condition on $\theta$ (\ProofWitness{fibre\_independent\_of\_base}); the base commutes for
...
of the generator of \eqref{eq:base}. \ProofWitnessBlock{M\_PCF\_eq\_pi\_div\_eps0, epsilon0\_from\_projection, bulk\_boundary\_isometry}
\end{proposition}
\begin{proof}
...
rotation versus translation---and not a morphism carrying one lattice onto another.
\ProofWitnessBlock{farishRot\_cube, farishRot\_diag, farishRot\_preserves\_dot, farishRot\_preserves\_norm,
...
\end{equation}
\ProofWitnessBlock{collapse\_norm\_product, norm\_product\_half}
\end{theorem}
...
\end{equation}
\ProofWitnessBlock{sigma\_from\_basel, sin\_cos\_mu}
\end{proposition}
...
\ProofWitnessBlock{factorial\_face, factorial\_six}
\end{proposition}
...
$3\,\golden^{\mu p\lng}=3\,(\sqrt2)^{p}$. \ProofWitnessBlock{mersenne\_mediated}
\end{proposition}
...
\end{equation}
multiplicative on $(\Z/5\Z)^{\times}$, with $\sum_{a=0}^{4}\chi_5(a)=0$. \ProofWitnessBlock{chi5, chi5\_even, chi5\_mul\_on\_units, chi5\_sum\_zero}
...
\ProofWitnessBlock{norm\_cosine\_phi, phi\_eq\_two\_cos\_pi\_fifth}
\end{proposition}
\begin{proof}
...
read in \eqref{eq:chi5-pentagon}, and $\golden$ is the value in both. \ProofWitnessBlock{sin\_ratio\_eq\_two\_cos, pentagon\_log\_signature}
...
$\OK$.  for the thirteen primes decided in Lean and the twenty-three verified numerically; \ProofWitnessBlock{mersenne\_bridge, phi\_pow\_fib}
the general congruence for every $q\neq2,5$ is Lucas'~\cite{Lucas1878}
\end{proposition}
...
Proposition~\ref{prop:fib-criterion} exploits. \ProofWitnessBlock{binet\_general, phi\_pow\_fib}
\end{definition}
...
\end{equation}
by \eqref{eq:binet}. \ProofWitnessBlock{psi\_golden, psi\_on\_powers, psi\_golden\_fib}
\end{definition}
...
\ProofWitnessBlock{psi\_functorial, psi\_one}
\end{proposition}
\begin{proof}
...
\end{equation}
\ProofWitnessBlock{eq:local-dedekind}%

\begin{theorem}[Basel value]\label{thm:basel}
$\zeta(2)=\pi^2/6$ (Euler). \ProofWitnessBlock{basel\_value}
\end{theorem}
...
\ProofWitnessBlock{even\_zeta\_bernoulli}
\NumericalWitnessBlock{thm:basel}
\end{theorem}
\begin{proof}
...
\end{equation}
by unique factorisation in $\Z$. \ProofWitnessBlock{regge\_dirichlet\_eq\_zeta, regge\_tower\_is\_euler\_product}
...
here rather than assumed. \ProofWitnessBlock{ideal\_norm\_k, ideal\_euler\_k, euler\_from\_ideals\_K}
...
\ProofWitnessBlock{degree\_formula\_K, ideal\_norm\_values\_K, euler\_from\_ideals\_K, von\_mangoldt\_k\_values, local\_dedekind\_ramified\_K, local\_dedekind\_split\_K, local\_dedekind\_inert\_K}
\end{proposition}
...
colimit, not the framework colimit of \S\ref{ssec:web}.  (absolute convergence for
$\Re s>1$~\cite{Riemann1859}
\end{proposition}
...
\ProofWitnessBlock{gamma\_r\_from\_tate}
%
...
\end{equation}
with $\golden^{\mu\lng}=\sqrt2$ of Proposition~\ref{prop:mediation}.  (the two classical \ProofWitness{sqrt2\_mediation, sqrt3\_mediation}
evaluations enter as classical~\cite{Jacobi1829}
\end{remark}
...
\ProofWitnessBlock{framework\_partition, framework\_partition\_eq\_completed\_zeta, places\_assembly}
\end{theorem}
\begin{proof}
...
type~\cite{ConnesConsaniSite}. Proposition~\ref{prop:archimedean} supplies it. \ProofWitnessBlock{chi5\_even}
\end{remark}
...
leg.
\ProofWitnessBlock{functional\_equation\_fixed\_line, fixed\_line\_is\_face\_phi, places\_assembly}
\end{theorem}
\begin{proof}
...
\end{equation}
and $M_K<2$. Granting Minkowski's theorem~\cite{Minkowski1896}---every ideal class contains an integral ideal of norm
at most $M_K$---that ideal has norm a positive integer below $2$, hence norm $1$, hence is the
unit ideal. Therefore
\begin{equation}\label{eq:hK}
  h_K=1 .
\end{equation}
\ProofWitnessBlock{class\_number\_one\_K, minkowski\_bound\_lt\_two}
\end{proposition}
...
proved; the formula itself is the input.
\ProofWitnessBlock{L1\_from\_class\_number\_formula, L1\_eq\_period\_over\_pi\_sqrt5}
%
\end{theorem}
...
it is the regulator of $\Q(\sqrt5)$, measured in the unit $\lambda_{\log}$.
\ProofWitnessBlock{lambda\_log\_mul\_regulator, sqrt5\_half\_L1\_eq\_regulator, entropy\_bridge}
...
entropy happens to equal one bit: it is the point where it is \emph{largest}.
\ProofWitnessBlock{entropy\_slack\_nonneg, binary\_entropy\_le\_one, binary\_entropy\_half}
...
\ProofWitnessBlock{odd\_zeta\_ratio}
...
value.
\ProofWitnessBlock{mu\_diagram\_commutes, mu\_faces\_pairwise\_eq}
\end{theorem}
...
\ProofWitnessBlock{factorial\_six, s3\_orders, sigma\_geom\_from\_S3}
\end{lemma}
...
\ProofWitnessBlock{sigma\_diagram\_commutes}
\end{theorem}
...
$\Re s=\mu$ (Theorem~\ref{thm:funct-eq}).
\ProofWitnessBlock{section2\_master, places\_assembly, fixed\_line\_is\_face\_phi, functional\_equation\_fixed\_line, s\_duality\_exact, s\_duality\_fixed\_point\_unique, theta\_from\_jacobi}
\end{theorem}
...
\end{equation}
\ProofWitnessBlock{conductor\_is\_derived}
\end{proposition}
...
\ProofWitnessBlock{two\_sq\_eq\_neg\_one\_mod\_five, five\_splits\_gaussian, conductor\_four\_is\_order\_of\_turn, binary\_period\_lives\_in\_the\_five\_component, four\_cocone}
\end{proposition}
...
\ProofWitnessBlock{conductor\_splits, fiber\_mod4, fiber\_mod5, chi5\_is\_the\_mod5\_fiber, mod4\_does\_not\_determine\_chi5, mod5\_does\_not\_determine\_chi4, conductor\_attribution}
\end{proposition}
...
\ProofWitnessBlock{spacing\_scale, scale\_injective\_in\_conductor}
\end{proposition}
...
arguments; what is proved is that the conductor entering them is forced.
\ProofWitnessBlock{envelope, envelope\_splits, field\_part\_distinguishes, counting\_law, derived\_unit, the\_unit\_is\_derived}
\end{proposition}
...
\ProofWitnessBlock{li\_modulus\_on\_line}
\end{proposition}
...
\ProofWitnessBlock{sdual\_mate, sdual\_conj\_commute, sdual\_mate\_involutive}
\end{definition}
...
\ProofWitnessBlock{sdual\_mate\_im, sdual\_mate\_re, sdual\_mate\_fixed\_iff}
\end{proposition}
...
\ProofWitnessBlock{extreme\_case\_is\_binary}
\end{proposition}
...
\ProofWitnessBlock{binary\_iff\_radial}
\end{proposition}
...
Poisson statistics do not. It is measured, not proved : over the $237$ spacings of $\gamma_1=14.13$ to \NumericalWitness{eq:repulsion}
...
\ProofWitnessBlock{repulsion\_excludes\_zero\_spacing, repulsion\_excludes\_the\_extreme\_case}
\end{proposition}
...
\ProofWitnessBlock{E\_iff\_no\_shared\_ordinate}
\end{theorem}
...
\ProofWitnessBlock{repulsion\_gives\_the\_modulus}
\NumericalWitnessBlock{prop:repulsion-modulus}
\end{proposition}
...
\eqref{eq:sdual-mate}, $\xi(\bar s)=\overline{\xi(s)}$, not established in this paper; and
the repulsion is measured . Everything between them is proved, and of the measurement only \NumericalWitness{eq:sdual-mate}
...
large $u$, which is decorrelation. Nothing below uses it.
\end{definition}

\begin{figure}[htb]\centering
\includegraphics[width=\linewidth]{fig8_sine_kernel.pdf}
\caption{The GUE pair-correlation kernel $K(u)=1-(\sin\pi u/\pi u)^{2}$ of Def.~\ref{def:pair-correlation}. Red point: $K(0){=}0$ (repulsion). Blue points: $K(n){=}0$, $n\in\Z\setminus\{0\}$ (no long-range correlation). The kernel tends to $1$ for large $u$ (decorrelation). The rug plot shows the $237$ measured unfolded spacings, clustering away from zero in the repulsion region.}
...
this section is the regime, not the reading. 
\end{remark}
...
\ProofWitnessBlock{tower\_spacing\_is\_the\_regulator}
\end{remark}
...
(\texttt{gamma\_r\_from\_tate}, \texttt{theta\_from\_jacobi}) and the combinatorial ones
...
under conjugation, which is \emph{not} established in this paper; and
\texttt{MinSpacing}, a positive minimal ordinate separation, which is measured rather than
proved . Of the second only its sign is used. Neither is an axiom: both are hypothesis \NumericalWitness{thm:zeros-apex}
...
the Polyakov action \eqref{eq:polyakov} (a classical~\cite{Polyakov1987} input);
...
backing sits at the local factors \eqref{eq:local-dedekind} that compose it. The figures are generated by the scripts in \texttt{scripts/figures/} and their assertions checked by the pytest suite (\texttt{pnpm run verify:py}).
% ===================================================================
