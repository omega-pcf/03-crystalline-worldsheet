...
% ---------------------------------------------------------------------
\noindent\emph{Verification tags:} $\Proof{...}$ formal proof (Lean), $\Numeric{...}$ numerical check,
$\Hyp{...}$ hypothesis from the literature, $\Conj$ proposed. Compound tags
($\Proof{...}\;\Numeric{...}$) mark a claim resting on more than one, and each input is named where it enters.
...
\ProofBlock{phi\_trace, phi\_norm, galois\_pair, phi\_discriminant}
\end{definition}
...
\ProofBlock{galois\_conj\_is\_one\_sub, galois\_involution\_is\_one\_sub}
\end{lemma}
...
the present section consumes. \Proof{mersenne\_bridge}
...
\ProofBlock{phi\_lt\_two, two\_lt\_phi\_sq, two\_not\_in\_golden\_lattice, anchor\_is\_exterior}
\end{proposition}
...
$\golden=2\cos(\pi/5)$. \Proof{phi\_eq\_two\_cos\_pi\_fifth}
\end{theorem}
\medskip
\begin{proof}
...
\end{equation}
\ProofBlock{M13\_pi\_eq\_five\_arccos}
\end{proposition}
...
\end{equation}
\ProofBlock{M1\_gamma\_integral}
\end{proposition}
...
\ProofBlock{M2\_gamma\_half, gaussian\_integral\_value, gaussian\_weight\_phi}
\end{lemma}
\medskip
...
\end{equation}
\ProofBlock{M3\_half\_factorial}
\end{theorem}
...
\end{equation}
\ProofBlock{gauss\_normalised\_iff, fourier\_amplitude\_iff, fourier\_width\_iff, selfdual\_gaussian\_unique}
\end{proposition}
\begin{proof}
...
\end{equation}
\ProofBlock{M4\_sqrt2, M4\_sqrt3}
\end{proposition}
...
only if $x=\tfrac12$. \Proof{faceInv\_apex}
\end{proposition}
...
depends directly on itself). \Proof{DistributedSelfReference}
\end{definition}
...
\ProofBlock{M6\_phi\_root\_unique, M6\_characteristic\_root\_is\_phi} \Numeric{eq:phisq}
\end{theorem}
...
\ProofBlock{spectral\_uniqueness}
\end{proposition}
...
of $\Rpcf$ and not of $\OK$. \Proof{phi\_is\_algebraic\_integer, half\_not\_algebraic\_integer}
...
should have the invariant named before it is used. \Proof{regulator\_K\_pos, period\_eq\_two\_pi\_regulator}
...
\ProofBlock{normP\_eq\_tan, normF\_eq\_cos, Omega\_eigenvalues}
\end{proposition}
\begin{proof}
...
\ProofBlock{triad\_re, triad\_trace\_zero, triad\_two\_products, triad\_det\_is\_phi\_pow}
\end{proposition}
...
$V^{\dagger}V=\mathbf1$. \Proof{bulkBoundary\_isometry}
\end{lemma}
\begin{proof}
...
\ProofBlock{spectral\_uniqueness, bulkBoundary\_isometry}
\end{remark}
...
\end{equation}
\ProofBlock{M8\_epsilon0\_from\_projection}
\end{theorem}
\begin{proof}
...
\ProofBlock{fibre\_independent\_of\_base, fibre\_monodromy, fibre\_reflection\_order\_two}
\end{proposition}
\begin{proof}
The fibre monodromy is $e^{i\pi}=-1$, of order two, $(-1)^2=1$, a $\Z/2$ datum that holds with
no condition on $\theta$ (\Proof{fibre\_independent\_of\_base}); the base commutes for
...
of the generator of \eqref{eq:base}. \Proof{M\_PCF\_eq\_pi\_div\_eps0}
%
\ProofBlock{M8\_epsilon0\_from\_projection, bulkBoundary\_isometry}%
\end{proposition}
\begin{proof}
...
\ProofBlock{farishRot\_cube, farishRot\_diag, farishRot\_preserves\_dot, farishRot\_preserves\_norm,
...
\end{equation}
\ProofBlock{M9\_collapse, M9\_eq\_half}
\end{theorem}
...
\end{equation}
\ProofBlock{M10\_sigma\_from\_basel, M10\_sin\_cos\_mu}
\end{proposition}
...
\ProofBlock{M11\_factorial\_face, M11\_factorial\_six}
\end{proposition}
...
$3\,\golden^{\mu p\lng}=3\,(\sqrt2)^{p}$. \Proof{M12\_mersenne\_mediated}
\end{proposition}
...
\end{equation}
multiplicative on $(\Z/5\Z)^{\times}$, with $\sum_{a=0}^{4}\chi_5(a)=0$. \Proof{chi5, chi5\_even, chi5\_mul\_on\_units, chi5\_sum\_zero}
...
\ProofBlock{norm\_cosine\_phi, phi\_eq\_two\_cos\_pi\_fifth}
\end{proposition}
\begin{proof}
...
read in \eqref{eq:chi5-pentagon}, and $\golden$ is the value in both. \Proof{sin\_ratio\_eq\_two\_cos, pentagon\_log\_signature}
...
$\OK$.  for the thirteen primes decided in Lean and the twenty-three verified numerically; \Proof{mersenne\_bridge, phi\_pow\_fib}
...
Proposition~\ref{prop:fib-criterion} exploits. \Proof{binet\_general, phi\_pow\_fib}
\end{definition}
...
\end{equation}
by \eqref{eq:binet}. \Proof{psiGolden, psi\_on\_powers, psi\_golden\_fib}
\end{definition}
...
\ProofBlock{psi\_functorial, psi\_one}
\end{proposition}
\begin{proof}
...
\end{equation}
\ProofBlock{eq:local-dedekind}%

\begin{theorem}[Basel value]\label{thm:basel}
$\zeta(2)=\pi^2/6$ (Euler). \Proof{M14\_basel}
\end{theorem}
...
(Theorem~\ref{thm:basel}). \Proof{even\_zeta\_bernoulli} \Numeric{thm:basel}
\end{theorem}
\begin{proof}
...
\end{equation}
by unique factorisation in $\Z$. \Proof{regge\_dirichlet\_eq\_zeta, regge\_tower\_is\_euler\_product}
...
here rather than assumed. \Proof{idealNormK, idealEulerK, euler\_from\_ideals\_K}
...
\ProofBlock{degree\_formula\_K, ideal\_norm\_values\_K, euler\_from\_ideals\_K, vonMangoldtK\_values, local\_dedekind\_ramified\_K, local\_dedekind\_split\_K, local\_dedekind\_inert\_K}
\end{proposition}
...
(Poisson summation enters as \Hyp{}) \Proof{gammaR\_from\_tate}
%
\ProofBlock{theta\_from\_jacobi, theta\_fixed\_point\_unique}%
\end{proposition}
\begin{proof}
...
\end{equation}
with $\golden^{\mu\lng}=\sqrt2$ of Proposition~\ref{prop:mediation}.  (the two classical \Proof{M4\_sqrt2, M4\_sqrt3}
...
\ProofBlock{framework\_partition, framework\_partition\_eq\_completed\_zeta, places\_assembly}
\end{theorem}
\begin{proof}
...
type~\cite{ConnesConsaniSite}. Proposition~\ref{prop:archimedean} supplies it. \Proof{chi5\_even}
\end{remark}
...
leg.
\ProofBlock{functional\_equation\_fixed\_line, fixed\_line\_is\_facePhi, places\_assembly}
\end{theorem}
\begin{proof}
...
\begin{equation}\label{eq:hK}
  h_K=1 .
\end{equation}
(Minkowski's theorem enters as \Hyp{}) \Proof{minkowski\_bound\_lt\_two}
%
\ProofBlock{class\_number\_one\_K}%
\end{proposition}
...
proved; the formula itself is the input.  (the class-number formula enters as \Hyp{}) \Proof{L1\_from\_class\_number\_formula, L1\_eq\_period\_over\_pi\_sqrt5}
%
\end{theorem}
...
it is the regulator of $\Q(\sqrt5)$, measured in the unit $\lambda_{\log}$. \Proof{lambda\_log\_mul\_regulator, sqrt5\_half\_L1\_eq\_regulator, entropy\_bridge}
...
entropy happens to equal one bit: it is the point where it is \emph{largest}. \Proof{entropy\_slack\_nonneg, binary\_entropy\_le\_one, binary\_entropy\_half}
...
rather than deferred.  (the global Dedekind factorisation enters as \Hyp{}) \Proof{odd\_zeta\_ratio}
...
value.
\ProofBlock{mu\_diagram\_commutes, mu\_faces\_pairwise\_eq}
\end{theorem}
...
\ProofBlock{M11\_factorial\_six, s3\_orders, sigma\_geom\_from\_S3}
\end{lemma}
...
\ProofBlock{sigma\_diagram\_commutes}
\end{theorem}
...
$\Re s=\mu$ (Theorem~\ref{thm:funct-eq}). \Proof{section2\_master, places\_assembly}
\ProofBlock{fixed\_line\_is\_facePhi, functional\_equation\_fixed\_line, places\_assembly, s\_duality\_exact, s\_duality\_fixed\_point\_unique, theta\_from\_jacobi}
\end{theorem}
...
\end{equation}
\ProofBlock{conductor\_is\_derived}
\end{proposition}
...
\ProofBlock{two\_sq\_eq\_neg\_one\_mod\_five, five\_splits\_gaussian, conductor\_four\_is\_order\_of\_turn, binary\_period\_lives\_in\_the\_five\_component, four\_cocone}
\end{proposition}
...
\ProofBlock{conductor\_splits, fiber\_mod4, fiber\_mod5, chi5\_is\_the\_mod5\_fiber, mod4\_does\_not\_determine\_chi5, mod5\_does\_not\_determine\_chi4, conductor\_attribution}
\end{proposition}
...
\ProofBlock{spacingScale, scale\_injective\_in\_conductor}
\end{proposition}
...
\ProofBlock{envelope, envelope\_splits, field\_part\_distinguishes, CountingLaw, DerivedUnit, the\_unit\_is\_derived}
\end{proposition}
...
\ProofBlock{li\_modulus\_on\_line}
\end{proposition}
...
\ProofBlock{sdualMate, sdual\_conj\_commute, sdualMate\_involutive}
\end{definition}
...
\ProofBlock{sdualMate\_im, sdualMate\_re, sdualMate\_fixed\_iff}
\end{proposition}
...
\ProofBlock{extreme\_case\_is\_binary}
\end{proposition}
...
\ProofBlock{binary\_iff\_radial}
\end{proposition}
...
Poisson statistics do not. It is measured, not proved : over the $237$ spacings of $\gamma_1=14.13$ to \Numeric{eq:repulsion}
...
\ProofBlock{repulsion\_excludes\_zero\_spacing, repulsion\_excludes\_the\_extreme\_case}
\end{proposition}
...
\ProofBlock{E\_iff\_no\_shared\_ordinate}
\end{theorem}
...
\ProofBlock{repulsion\_gives\_the\_modulus}
\end{proposition}
...
the repulsion is measured . Everything between them is proved, and of the measurement only \Numeric{eq:sdual-mate}
...
\ProofBlock{tower\_spacing\_is\_the\_regulator}
\end{remark}
...
proved . Of the second only its sign is used. Neither is an axiom: both are hypothesis \Numeric{thm:zeros-apex}
...
the Polyakov action \eqref{eq:polyakov} (an \Hyp{} input);
