...
\ProofWitnessBlock{gamma\_r\_from\_tate, theta\_from\_jacobi, theta\_fixed\_point\_unique}%
\end{proposition}
\begin{proof}
...
\end{equation}
with $\golden^{\mu\lng}=\sqrt2$ of Proposition~\ref{prop:mediation}. (the two classical evaluations enter as classical~\cite{Jacobi1829})
\end{remark}
...
Poisson statistics do not. It is measured, not proved: over the $237$ spacings of $\gamma_1=14.13$ to
...
the repulsion is measured. Everything between them is proved, and of the measurement only
...
proved. Of the second only its sign is used. Neither is an axiom: both are hypothesis
