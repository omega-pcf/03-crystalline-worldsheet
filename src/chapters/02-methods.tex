...
% ---------------------------------------------------------------------
\noindent\emph{Tiers, used from here on:} \tier{P} proved in Lean; \tier{L} assumed from the
literature and cited; \tier{N} verified numerically; \tier{C} proposed. Compound tags mark a claim
resting on more than one, and each classical input is named where it enters.
...
\E^3=\{(x,y,\golden y)\}.
\end{equation}
\LeanWrap{phi\_sq, phi\_sq\_plus\_inv\_sq\_eq\_n}
...
\tier{P} \Lean{phi\_trace, phi\_norm, galois\_pair, phi\_discriminant}
\end{definition}
...
\tier{P} \Lean{galois\_conj\_is\_one\_sub, galois\_involution\_is\_one\_sub}
\end{lemma}
...
the present section consumes. \LeanWrap{mersenne\_bridge}
...
\tier{P} \Lean{phi\_lt\_two, two\_lt\_phi\_sq, two\_not\_in\_golden\_lattice, anchor\_is\_exterior}
\end{proposition}
...
$\golden=2\cos(\pi/5)$. \tier{P} \Lean{phi\_eq\_two\_cos\_pi\_fifth}
\end{theorem}
\medskip
\begin{proof}
...
\end{equation}
\tier{P} \Lean{gauss\_normalised\_iff, fourier\_amplitude\_iff, fourier\_width\_iff, selfdual\_gaussian\_unique}
\end{proposition}
\begin{proof}
...
only if $x=\tfrac12$. \tier{P} \Lean{faceInv\_apex}
\end{proposition}
...
depends directly on itself). \LeanWrap{DistributedSelfReference}
\tier{P}
\end{definition}
...
of $\Rpcf$ and not of $\OK$. \tier{P}
\Lean{phi\_is\_algebraic\_integer, half\_not\_algebraic\_integer}
\end{proposition}
...
should have the invariant named before it is used. \tier{P}
\Lean{regulator\_K\_pos, period\_eq\_two\_pi\_regulator}
\end{definition}

\begin{equation}\label{eq:omega-norms}
\Omega=P\cdot C\cdot F .
\end{equation}
\LeanWrap{Omega\_eigenvalues, normP, normP\_eq\_tan}
...
\tier{P} \Lean{normP\_eq\_tan, normF\_eq\_cos, Omega\_eigenvalues}
\end{proposition}
\begin{proof}
...
\tier{P} \Lean{triad\_re, triad\_trace\_zero, triad\_two\_products, triad\_det\_is\_phi\_pow}
\end{proposition}
...
$V^{\dagger}V=\mathbf1$. \LeanWrap{bulkBoundary\_isometry}
\tier{P}
\end{lemma}
\begin{proof}
...
\tier{P} \Lean{spectral\_uniqueness, bulkBoundary\_isometry}
\end{remark}

\begin{definition}[PCF projection]\label{def:projpcf}
\LeanWrap{projection\_PCF}
...
\eqref{eq:norms-derived}. \tier{P} \Lean{M7\_oplus\_formula}
\end{remark}
...
\end{equation}
\tier{P} \Lean{M8\_epsilon0\_from\_projection}
\end{theorem}
\begin{proof}
...
\end{equation}
\LeanWrap{M\_PCF\_eq\_pi\_div\_eps0}
...
of the generator of \eqref{eq:base}. \tier{P}
\LeanFor{eq:closure}{M\_PCF\_eq\_pi\_div\_eps0}%
\LeanFor{eq:mpcf-invariants}{M8\_epsilon0\_from\_projection, bulkBoundary\_isometry}%
\end{proposition}
\begin{proof}
...
rotation versus translation---and not a morphism carrying one lattice onto another. \tier{P}
\Lean{farishRot\_cube, farishRot\_diag, farishRot\_preserves\_dot, farishRot\_preserves\_norm,
...
\end{equation}
\LeanWrap{phi\_pow\_fib, mersenne\_bridge}

\begin{proposition}[Mediated Mersenne]\label{prop:mersenne-med}
$3\,\golden^{\mu p\lng}=3\,(\sqrt2)^{p}$. \LeanWrap{M12\_mersenne\_mediated}
\tier{P}
\end{proposition}
...
\end{equation}
multiplicative on $(\Z/5\Z)^{\times}$, with $\sum_{a=0}^{4}\chi_5(a)=0$. \tier{P}
\Lean{chi5, chi5\_even, chi5\_mul\_on\_units, chi5\_sum\_zero}
\end{definition}
...
\tier{P} \Lean{norm\_cosine\_phi, phi\_eq\_two\_cos\_pi\_fifth}
\end{proposition}
\begin{proof}
...
read in \eqref{eq:chi5-pentagon}, and $\golden$ is the value in both. \tier{P}
\Lean{sin\_ratio\_eq\_two\_cos, pentagon\_log\_signature}
\end{proposition}
...
$\OK$. \tier{P} for the thirteen primes decided in Lean and the twenty-three verified numerically;
the general congruence for every $q\neq2,5$ is Lucas' and enters as \tier{L}
\Lean{fib\_criterion\_cases, chi5\_split\_inert\_mod20}
\end{proposition}
...
Proposition~\ref{prop:fib-criterion} exploits. \tier{P} \Lean{binet\_general, phi\_pow\_fib}
\end{definition}
...
\end{equation}
by \eqref{eq:binet}. \tier{P} \Lean{psiGolden, psi\_on\_powers, psi\_golden\_fib}
\end{definition}
...
\tier{P} \Lean{psi\_functorial, psi\_one}
\end{proposition}
\begin{proof}
...
\end{equation}
\LeanInline{eq:local-dedekind}%

\begin{theorem}[Basel value]\label{thm:basel}
$\zeta(2)=\pi^2/6$ (Euler). \LeanWrap{M14\_basel}
\tier{P}
\end{theorem}
...
(Theorem~\ref{thm:basel}). \tier{P/N} \Lean{even\_zeta\_bernoulli}
\end{theorem}
\begin{proof}
...
\end{equation}
by unique factorisation in $\Z$. \tier{P}
\Lean{regge\_dirichlet\_eq\_zeta, regge\_tower\_is\_euler\_product}
\end{proposition}
...
here rather than assumed. \tier{P}
\Lean{idealNormK, idealEulerK, euler\_from\_ideals\_K}
\end{definition}
...
\tier{P}
\LeanFor{eq:splitting}{degree\_formula\_K, ideal\_norm\_values\_K}%
\LeanFor{eq:local-K}{euler\_from\_ideals\_K}%
\LeanFor{eq:LambdaK}{vonMangoldtK\_values}%
\LeanFor{eq:local-dedekind}{local\_dedekind\_ramified\_K, local\_dedekind\_split\_K, local\_dedekind\_inert\_K}%
\end{proposition}
...
colimit, not the framework colimit of \S\ref{ssec:web}. \tier{P} (absolute convergence for
$\Re s>1$ enters as \tier{L})
\Lean{coeff\_stabilises\_K, colimit\_exists\_K}
\end{proposition}
...
\tier{P} (Poisson summation enters as \tier{L})
\LeanFor{eq:gammaR-mellin}{gammaR\_from\_tate}%
\LeanFor{eq:theta-lattice}{theta\_from\_jacobi, theta\_fixed\_point\_unique}%
\end{proposition}
\begin{proof}
...
\end{equation}
with $\golden^{\mu\lng}=\sqrt2$ of Proposition~\ref{prop:mediation}. \tier{P} (the two classical
evaluations enter as \tier{L})
\end{remark}
...
\tier{P} \Lean{framework\_partition, framework\_partition\_eq\_completed\_zeta, places\_assembly}
\end{theorem}
\begin{proof}
...
type~\cite{ConnesConsaniSite}. Proposition~\ref{prop:archimedean} supplies it. \tier{P} \Lean{chi5\_even}
\end{remark}
...
leg.
\tier{P} \Lean{theta\_from\_jacobi, s\_duality\_exact, s\_duality\_fixed\_point\_unique,
...
are the same map.
\Lean{s\_duality\_exact, faceInv\_apex, faceGalois\_apex, fixed\_line\_is\_facePhi}
\end{remark}
...
\begin{equation}\label{eq:hK}
  h_K=1 .
\end{equation}
\tier{P} (Minkowski's theorem enters as \tier{L})
\LeanFor{eq:minkowski}{minkowski\_bound\_lt\_two}%
\LeanFor{eq:hK}{class\_number\_one\_K}%
\end{proposition}
...
proved; the formula itself is the input. \tier{P} (the class-number formula enters as \tier{L})
\LeanFor{eq:L1}{L1\_from\_class\_number\_formula, L1\_eq\_period\_over\_pi\_sqrt5}%
\end{theorem}
...
it is the regulator of $\Q(\sqrt5)$, measured in the unit $\lambda_{\log}$. \tier{P}
\Lean{lambda\_log\_mul\_regulator, sqrt5\_half\_L1\_eq\_regulator, entropy\_bridge}
\end{theorem}
...
entropy happens to equal one bit: it is the point where it is \emph{largest}. \tier{P}
\Lean{entropy\_slack\_nonneg, binary\_entropy\_le\_one, binary\_entropy\_half}
\end{proposition}
\begin{proof}
...
rather than deferred. \tier{P} (the global Dedekind factorisation enters as \tier{L})
\Lean{odd\_zeta\_ratio}
\end{theorem}
...
\tier{P} \Lean{M11\_factorial\_six, s3\_orders, sigma\_geom\_from\_S3}
\end{lemma}
...
$\Re s=\mu$ (Theorem~\ref{thm:funct-eq}). \LeanWrap{section2\_master, places\_assembly}
\tier{P}
\end{theorem}
...
\end{equation}
\tier{P} \Lean{conductor\_is\_derived}
\end{proposition}
...
\tier{P} \Lean{two\_sq\_eq\_neg\_one\_mod\_five, five\_splits\_gaussian, conductor\_four\_is\_order\_of\_turn, binary\_period\_lives\_in\_the\_five\_component, four\_cocone}
\end{proposition}
...
\tier{P} \Lean{conductor\_splits, fiber\_mod4, fiber\_mod5, chi5\_is\_the\_mod5\_fiber, mod4\_does\_not\_determine\_chi5, mod5\_does\_not\_determine\_chi4, conductor\_attribution}
\end{proposition}
...
\tier{P} \Lean{spacingScale, scale\_injective\_in\_conductor}
\end{proposition}
...
arguments \tier{C}; what is proved is that the conductor entering them is forced.
\tier{P} \Lean{envelope, envelope\_splits, field\_part\_distinguishes, CountingLaw, DerivedUnit, the\_unit\_is\_derived}
\end{proposition}
...
\tier{P} \Lean{li\_modulus\_on\_line}
\end{proposition}
...
\tier{P} \Lean{sdualMate, sdual\_conj\_commute, sdualMate\_involutive}
\end{definition}
...
\tier{P} \Lean{sdualMate\_im, sdualMate\_re, sdualMate\_fixed\_iff}
\end{proposition}
...
\tier{P} \Lean{extreme\_case\_is\_binary}
\end{proposition}
...
\tier{P} \Lean{binary\_iff\_radial}
\end{proposition}
...
Poisson statistics do not. It is measured, not proved \tier{N}: over $237$ unfolded spacings with
$\gamma\leq329.30$ the minimum is $0.2307$; none falls below $0.10$, where independence would put
several; the fit to GUE against Poisson favours GUE by a factor $7.6$; and the unfolded mean is
$1.0073$ with the proper conductor against $0.8899$ with a foreign one---which is
Proposition~\ref{prop:scale} showing in the data. Only the \emph{sign} of $m$ enters anything below.
\Lean{Repulsion}
\end{definition}
...
\tier{P} \Lean{repulsion\_excludes\_zero\_spacing, repulsion\_excludes\_the\_extreme\_case}
\end{proposition}
...
\tier{P} \Lean{E\_iff\_no\_shared\_ordinate}
\end{theorem}
...
\tier{P} \Lean{repulsion\_gives\_the\_modulus}
\end{proposition}
...
\eqref{eq:sdual-mate}, $\xi(\bar s)=\overline{\xi(s)}$, not established in this paper \tier{C}; and
the repulsion is measured \tier{N}. Everything between them is proved, and of the measurement only
the \emph{sign} enters.
\Lean{XiConjClosed, Repulsion, zeros\_on\_the\_apex}
\end{theorem}
...
large $u$, which is decorrelation. Nothing below uses it. \tier{C}
\Lean{sineKernel, sineKernel\_at\_nonzero\_integer}
\end{definition}
...
them. \tier{$\varnothing$}
\Lean{SmoothDensity, FineFluctuations, JointDensity, FormFactor}
\end{remark}
...
this section is the regime, not the reading. \tier{O}
\end{remark}
...
\tier{P} \Lean{tower\_spacing\_is\_the\_regulator}
\end{remark}
...
under conjugation, which is \emph{not} established in this paper \tier{C}; and
\texttt{MinSpacing}, a positive minimal ordinate separation, which is measured rather than
proved \tier{N}. Of the second only its sign is used. Neither is an axiom: both are hypothesis
arguments in the type of Theorem~\ref{thm:zeros-apex}. Two decision procedures use
\texttt{native\_decide} rather than \texttt{decide}---the multiplicativity of $\chi_5$ over the
units, and the sweep of $16\times17\times18$ triples behind
...
What remains to be confirmed against the installed Mathlib version is the spelling of the library
lemmas the proofs call. Grouped by what they serve:
...
\item \emph{The eigenvalue triad} (Proposition~\ref{prop:triad-invariants}):
  ...
  \texttt{Complex.exp\_two\_pi\_mul\_I}, \texttt{Real.cos\_pi\_add}, \texttt{Real.cos\_pi\_sub},
...
\item \emph{Theta and Poisson} (\eqref{eq:theta-lattice}):
  \texttt{Real.tsum\_eq\_tsum\_fourierIntegral}, \texttt{jacobiTheta}.
\item \emph{Pentagon and Fibonacci} (\S\ref{ssec:spectrum}, \S\ref{ssec:tower}):
  \texttt{Real.cos\_pi\_sub}, \texttt{Real.cos\_two\_mul}, \texttt{goldenRatio}, \texttt{Nat.fib}.
...
\end{itemize}
\noindent The list and the file cannot drift apart, and the guarantee is now stronger than a
convention: since \texttt{lake build} completes, any renaming or removal of these lemmas in a
future Mathlib breaks the build rather than passing silently. The numerical companion is \texttt{CW6\_complete\_verify\_v2.py},
whose checks are indexed by the \TeX{} labels of this file: of the $162$ labelled equations,
$147$ carry a Lean proof and $108$ a numerical check, $102$ carrying both---the checks evaluating
identities at up to $25$-digit precision and, where a claim is a correspondence with measurement,
comparing against the measured value. The nine carrying neither a proof nor a check of their
own are the six correspondence statements of the duality list
\eqref{eq:bridge-S}--\eqref{eq:bridge-fixed}; the class-number formula \eqref{eq:cnf}, whose
Lean form is the hypothesis \texttt{hCNF} of \texttt{L1\_from\_class\_number\_formula};
the Polyakov action \eqref{eq:polyakov} (an \tier{L} input); and the
