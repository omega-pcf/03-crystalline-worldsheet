...
the present section consumes. \ProofWitness{mersenne\_bridge}
...
$\golden=2\cos(\pi/5)$. \ProofWitness{phi\_eq\_two\_cos\_pi\_fifth}
\end{theorem}
\medskip
\begin{proof}
...
only if $x=\tfrac12$. \ProofWitness{faceInv\_apex}
\end{proposition}
...
depends directly on itself). \ProofWitness{DistributedSelfReference}
\end{definition}
...
of $\Rpcf$ and not of $\OK$. \ProofWitness{phi\_is\_algebraic\_integer, half\_not\_algebraic\_integer}
...
should have the invariant named before it is used. \ProofWitness{regulator\_K\_pos, period\_eq\_two\_pi\_regulator}
...
$V^{\dagger}V=\mathbf1$. \ProofWitness{bulkBoundary\_isometry}
\end{lemma}
\begin{proof}
...
\eqref{eq:norms-derived}. \ProofWitness{M7\_oplus\_formula}
\end{remark}
...
of the generator of \eqref{eq:base}. \ProofWitness{M\_PCF\_eq\_pi\_div\_eps0}
%
...
$3\,\golden^{\mu p\lng}=3\,(\sqrt2)^{p}$. \ProofWitness{M12\_mersenne\_mediated}
\end{proposition}
...
\end{equation}
multiplicative on $(\Z/5\Z)^{\times}$, with $\sum_{a=0}^{4}\chi_5(a)=0$. \ProofWitness{chi5, chi5\_even, chi5\_mul\_on\_units, chi5\_sum\_zero}
...
read in \eqref{eq:chi5-pentagon}, and $\golden$ is the value in both. \ProofWitness{sin\_ratio\_eq\_two\_cos, pentagon\_log\_signature}
...
$\OK$.  for the thirteen primes decided in Lean and the twenty-three verified numerically; \ProofWitness{mersenne\_bridge, phi\_pow\_fib}
...
Proposition~\ref{prop:fib-criterion} exploits. \ProofWitness{binet\_general, phi\_pow\_fib}
\end{definition}
...
\end{equation}
by \eqref{eq:binet}. \ProofWitness{psiGolden, psi\_on\_powers, psi\_golden\_fib}
\end{definition}
...
$\zeta(2)=\pi^2/6$ (Euler). \ProofWitness{M14\_basel}
\end{theorem}
...
\end{equation}
by unique factorisation in $\Z$. \ProofWitness{regge\_dirichlet\_eq\_zeta, regge\_tower\_is\_euler\_product}
...
here rather than assumed. \ProofWitness{idealNormK, idealEulerK, euler\_from\_ideals\_K}
