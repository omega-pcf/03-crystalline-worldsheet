...
\end{equation}
\ProofWitnessBlock{M13\_pi\_eq\_five\_arccos}
\end{proposition}
...
\end{equation}
\ProofWitnessBlock{M1\_gamma\_integral}
\end{proposition}
...
\ProofWitnessBlock{M2\_gamma\_half, gaussian\_integral\_value, gaussian\_weight\_phi}
\end{lemma}
\medskip
...
\end{equation}
\ProofWitnessBlock{M3\_half\_factorial}
\end{theorem}
...
\ProofWitnessBlock{M6\_phi\_root\_unique, M6\_characteristic\_root\_is\_phi}
...
\end{equation}
\ProofWitnessBlock{M8\_epsilon0\_from\_projection}
\end{theorem}
\begin{proof}
...
of the generator of \eqref{eq:base}. \ProofWitnessBlock{M\_PCF\_eq\_pi\_div\_eps0, M8\_epsilon0\_from\_projection, bulk\_boundary\_isometry}
\end{proposition}
\begin{proof}
...
\end{equation}
\ProofWitnessBlock{M9\_collapse, M9\_eq\_half}
\end{theorem}
...
\end{equation}
\ProofWitnessBlock{M10\_sigma\_from\_basel, M10\_sin\_cos\_mu}
\end{proposition}
...
\ProofWitnessBlock{M11\_factorial\_face, M11\_factorial\_six}
\end{proposition}
...
$3\,\golden^{\mu p\lng}=3\,(\sqrt2)^{p}$. \ProofWitnessBlock{M12\_mersenne\_mediated}
\end{proposition}
...
$\zeta(2)=\pi^2/6$ (Euler). \ProofWitnessBlock{M14\_basel}
\end{theorem}
...
\end{equation}
with $\golden^{\mu\lng}=\sqrt2$ of Proposition~\ref{prop:mediation}.  (the two classical \ProofWitness{M4\_sqrt2, M4\_sqrt3}
...
proved; the formula itself is the input.  (the class-number formula enters as \HypWitness{class\_number\_formula}) \ProofWitness{L1\_from\_class\_number\_formula, L1\_eq\_period\_over\_pi\_sqrt5}
%
\end{theorem}
...
it is the regulator of $\Q(\sqrt5)$, measured in the unit $\lambda_{\log}$. \ProofWitness{lambda\_log\_mul\_regulator, sqrt5\_half\_L1\_eq\_regulator, entropy\_bridge}
...
\ProofWitnessBlock{M11\_factorial\_six, s3\_orders, sigma\_geom\_from\_S3}
\end{lemma}
