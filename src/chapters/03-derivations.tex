...
\S\ref{ssec:construction} used to fix $\tau=i$. \P{selfdual_gaussian_unique}
...
route \eqref{eq:sigma-basel} gives $n^{2}/6$, which meets $\tfrac32$ only at $n=3$. \P{three_halves_unique, arity_control_three}
...
Corollary~\ref{cor:bridge-angle} divides by $\cos\alpha$. \P{spectral_angle_tan, spectral_angle_tower_ratio}
...
put $55$ at the tower ceiling $\sigma=2n$, where the saturation entropy is largest. \P{fib_adjacent_le_one, Nmodes_six_ne_fib} \N{thm:fib-min}
...
it supports are established in \S\ref{sec:dof}. \P{towerHam\_isHermitian, towerHam\_bounded\_below}
...
The physical content is the identification of the index set, not the arithmetic identity. \P{regge_dirichlet_eq_zeta, regge_tower_is_euler_product}
...
read at $\tau=i$ and at general $t$. \P{theta_from_jacobi, theta_fixed_point_unique}
...
generations---which remains structural. \P{colour\_ratio} The ultraviolet is regulated in two complementary
...
$\chi(\mathbb{CP}^2)=3=n$ \P{hopf\_latitude, hopf\_from\_clifford}
The same arity-$3$ of
...
$H_5=\{0,1\}^5$, $|H_5|=2^{5}$ \P{hypercube\_card}
with
$\Lambda_\sigma=\golden^{\sigma}\Lambda_{\PCF}$ and the saturation entropy
$S(\sigma)=\pi\golden^{\sigma}$ \eqref{eq:tower-modes}. \P{conductor_attribution, four_cocone, S_tower}
...
\P{eisenstein\_omega, Omega\_eigenvalues, a2\_minimal\_vectors\_exactly\_six};
...
$S_3\to\Z_4$ between them preserves $|\Om|=\tfrac12$ \P{holographic\_area} and fixes $\tau=i$:
...
(Remark~\ref{rmk:spectral-origin}): \P{bulkBoundary_isometry, spectral_uniqueness, bulkBoundary_isometry}
...
\begin{align}
  ...
  \text{(asymptotic safety)}\quad & \text{UV fixed point}.\P{c\_tendsto\_two} \label{eq:bridge-fixed}
\end{align}
...
\P{bridge\_cocycle, bridge\_inverse, bridge\_compose} % ADD fase 1: respaldo Lean del cociclo
\P{}
...
coincidence. \P{bridge_eq_tan_form, bridge_pi_quarter, spectral_angle_tan}
...
for the four items; the Virasoro and brane-bouquet constructions themselves enter cited, \P{}
