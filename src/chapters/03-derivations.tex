...
in \texttt{CW6\_complete\_verify\_v2.py} (\S\ref{ssec:status}), at $25$-digit precision where
the identity requires it.
...
\end{equation}
\LeanFor{eq:gamma-half}{M2\_gamma\_half, M1\_gamma\_integral}%
...
\end{equation}
\LeanFor{eq:gaussian}{selfdual\_gaussian\_unique, gaussian\_integral\_value}%
...
\S\ref{ssec:construction} used to fix $\tau=i$. \tier{P}
\Lean{selfdual\_gaussian\_unique}
\end{remark}
...
carries the fibre.
\tier{P} \Lean{fibre\_monodromy, fibre\_reflection\_order\_two, fibre\_independent\_of\_base}
\end{proposition}
...
route \eqref{eq:sigma-basel} gives $n^{2}/6$, which meets $\tfrac32$ only at $n=3$. \tier{P}
\LeanFor{eq:spectral-invariants}{three\_halves\_unique, arity\_control\_three}%
...
\Lean{spectral\_angle\_tan}. The invariants \eqref{eq:spectral-invariants} are not chosen but
forced: the pair $(\sigma,\mu)$ with $\sigma+\mu=2$ and $\sigma\mu=\tfrac34$ has the unique
solution $(\sigma,\mu)=(\tfrac32,\tfrac12)$ \LeanWrap{spectral\_uniqueness}, i.e.\ $\sigma=\tfrac32$
\Lean{sigma\_three}, the same arity $3$ and modulus $\tfrac12$ forced by the No-Diagonal theorem
...
Corollary~\ref{cor:bridge-angle} divides by $\cos\alpha$. \tier{P}
\LeanFor{eq:spectral-angle}{spectral\_angle\_tan, spectral\_angle\_tower\_ratio}%
\LeanFor{eq:spectral-surface}{spectral\_surface\_closed}%
\Lean{spectral\_angle\_lt\_pi\_div\_two, cos\_spectral\_angle\_pos}
\end{proposition}
\begin{proof}
...
\begin{equation}\label{eq:bridge-BH}
  ...
  \SBH=\frac{A}{4\GN}.
\end{equation}
\Lean{binary\_entropy\_half, BH\_factor, holographic\_area\_factor}
...
(Verified in \texttt{CW6\_complete\_verify\_v2.py}: $H(\tfrac12)=1$, $c=3$, $\GN=\tfrac12$.)
...
\Lean{four\_cocone, conductor\_attribution}
...
\end{equation}
\tier{P}\LeanFor{eq:tower-mediation}{M4\_sqrt2, M4\_sqrt3}%
...
\end{equation}
\LeanFor{eq:tower-modes}{S\_tower}% The count and its golden ratio are shown in Fig.~\ref{fig:tower-modes}.
...
put $55$ at the tower ceiling $\sigma=2n$, where the saturation entropy is largest. \tier{P/N}
\Lean{fib\_adjacent\_le\_one, Nmodes\_six\_ne\_fib}
\end{remark}
...
it supports are established in \S\ref{sec:dof}. \LeanWrap{towerHam\_isHermitian, towerHam\_bounded\_below}
...
(App.~\ref{app:web}).\,\LeanWrap{modulus\_Omega, cocone\_property}
...
Regge tower.
\LeanWrap{regge\_tower\_is\_euler\_product, regge\_dirichlet\_eq\_zeta, regge\_residue\_ne\_zero, regge\_residue\_degree}
...
The physical content is the identification of the index set, not the arithmetic identity. \tier{P}
...
read at $\tau=i$ and at general $t$. \tier{P}
\Lean{theta\_from\_jacobi, theta\_fixed\_point\_unique}
\end{remark}
...
generations---which remains structural. \LeanWrap{colour\_ratio} The ultraviolet is regulated in two complementary
...
\end{enumerate}
(Verified in \texttt{CW6\_complete\_verify\_v2.py}: $\Gamma(\tfrac12)^{2}=\pi$, Regge poles,
...
\end{equation}
\LeanFor{eq:throat}{ricciCoeff, ricciScalar, einsteinCoeff}%
...
\tier{P} \Lean{R\_scalar\_pcf, Lambda5\_pcf, G\_AB\_pcf, BF\_value}
Its companion five-sphere carries the Hopf chain $S^1\hookrightarrow S^5\to\mathbb{CP}^2$ with
$\chi(\mathbb{CP}^2)=3=n$ \LeanWrap{hopf\_latitude, hopf\_from\_clifford}, the same arity-$3$ of
...
$H_5=\{0,1\}^5$, $|H_5|=2^{5}$ \LeanWrap{hypercube\_card}, with
$\Lambda_\sigma=\golden^{\sigma}\Lambda_{\PCF}$ and the saturation entropy
$S(\sigma)=\pi\golden^{\sigma}$ \eqref{eq:tower-modes}. \tier{P}
\begin{figure}[htb]\centering
\includegraphics[width=\linewidth]{fig5_three_panel.pdf}
...
\Lean{eisenstein\_omega, Omega\_eigenvalues, a2\_minimal\_vectors\_exactly\_six}; the same modulus on the
...
$S_3\to\Z_4$ between them preserves $|\Om|=\tfrac12$ \LeanWrap{holographic\_area} and fixes $\tau=i$:
...
\eqref{eq:isometry} requires. \tier{P} for the two lattices and the mediation; the GKPW dictionary
of \eqref{eq:isometry} enters cited, \tier{L}~\cite{Maldacena98,GKP98,Witten98}.
\begin{figure}[htb]\centering
\includegraphics[width=\linewidth]{fig4_top_down.pdf}
...
\LeanFor{eq:tower-autosimilar}{tower\_frobenius\_common}%
...
(Remark~\ref{rmk:spectral-origin}): \tier{P}
...
\LeanFor{eq:brown-henneaux}{worldline\_modulus\_is\_GN, hinge\_central\_charge}%
...
\texttt{CW6\_complete\_verify\_v2.py}: $|\Om|_{\sigma}=\tfrac12\ \forall\sigma$,
...
at $\tau=i$, the fixed point of S-duality \LeanWrap{s\_duality\_fixes\_i}, and the scale by the
certainty principle $\varepsilon_0 M_{\PCF}=\pi$ \Lean{certainty\_principle}. \tier{P}
...
\end{equation}
\LeanWrap{cocone\_property, colimit\_universal, framework\_colimit, GKP\_shared}
...
\tier{P} \Lean{theta\_from\_jacobi, theta\_fixed\_point\_unique}
\end{remark}
...
shared HS/Maldacena signature) are checked numerically in \texttt{CW6\_complete\_verify\_v2.py}
...
coincidence. \tier{P}
\Lean{bridge\_eq\_tan\_form, bridge\_pi\_quarter, spectral\_angle\_tan}
\end{corollary}
\begin{proof}
...
\tier{P} for the four items; the Virasoro and brane-bouquet constructions themselves enter cited,
\tier{L}~\cite{BrownHenneaux86,Witten95}.
\Lean{strings\_and\_holography\_commute\_at\_c, virasoro\_charge\_is\_the\_modulus, ladder\_count\_is\_the\_turn, one\_recurrence\_covers\_the\_ladder, no\_privileged\_level, three\_reached\_by\_faithful\_isometry, the\_reverse\_face}
\end{proposition}
