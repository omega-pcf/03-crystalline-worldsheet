...
\S\ref{ssec:construction} used to fix $\tau=i$. \Proof{selfdual_gaussian_unique}
...
carries the fibre.
\Proof{fibre_monodromy, fibre_reflection_order_two, fibre_independent_of_base}
\end{proposition}
...
route \eqref{eq:sigma-basel} gives $n^{2}/6$, which meets $\tfrac32$ only at $n=3$. \Proof{three_halves_unique, arity_control_three}
...
Corollary~\ref{cor:bridge-angle} divides by $\cos\alpha$. \Proof{spectral_angle_tan, spectral_angle_tower_ratio}
...
put $55$ at the tower ceiling $\sigma=2n$, where the saturation entropy is largest. \Proof{fib_adjacent_le_one, Nmodes_six_ne_fib} \Numeric{thm:fib-min}
...
it supports are established in \S\ref{sec:dof}. \Proof{towerHam\_isHermitian, towerHam\_bounded\_below}
...
The physical content is the identification of the index set, not the arithmetic identity. \Proof{regge_dirichlet_eq_zeta, regge_tower_is_euler_product}
...
read at $\tau=i$ and at general $t$. \Proof{theta_from_jacobi, theta_fixed_point_unique}
...
generations---which remains structural. \Proof{colour\_ratio} The ultraviolet is regulated in two complementary
...
\Proof{R_scalar_pcf, Lambda5_pcf, G_AB_pcf, BF_value}
Its companion five-sphere carries the Hopf chain $S^1\hookrightarrow S^5\to\mathbb{CP}^2$ with
$\chi(\mathbb{CP}^2)=3=n$ \Proof{hopf\_latitude, hopf\_from\_clifford}
The same arity-$3$ of
...
$H_5=\{0,1\}^5$, $|H_5|=2^{5}$ \Proof{hypercube\_card}
with
$\Lambda_\sigma=\golden^{\sigma}\Lambda_{\PCF}$ and the saturation entropy
$S(\sigma)=\pi\golden^{\sigma}$ \eqref{eq:tower-modes}. \Proof{conductor_attribution, four_cocone, S_tower}
...
\Proof{eisenstein\_omega, Omega\_eigenvalues, a2\_minimal\_vectors\_exactly\_six};
...
$S_3\to\Z_4$ between them preserves $|\Om|=\tfrac12$ \Proof{holographic\_area} and fixes $\tau=i$:
...
\eqref{eq:isometry} requires.  for the two lattices and the mediation; the GKPW dictionary \Proof{holographic_area, hopf_from_clifford, hopf_latitude, G_AB_pcf, Lambda5_pcf, R_scalar_pcf, BF_value, Omega_eigenvalues, a2_minimal_vectors_exactly_six, eisenstein_omega, hypercube_card, holographic_area, hopf_from_clifford, hopf_latitude, G_AB_pcf, Lambda5_pcf, R_scalar_pcf, BF_value, Omega_eigenvalues, a2_minimal_vectors_exactly_six, eisenstein_omega, hypercube_card}
of \eqref{eq:isometry} enters cited, ~\cite{Maldacena98,GKP98,Witten98}. \Hyp[\cite{Maldacena98, GKP98, Witten98}]
...
(Remark~\ref{rmk:spectral-origin}): \Proof{bulkBoundary_isometry, spectral_uniqueness, bulkBoundary_isometry}
...
\begin{align}
  ...
  \text{(asymptotic safety)}\quad & \text{UV fixed point}.\Proof{c\_tendsto\_two} \label{eq:bridge-fixed}
\end{align}
...
\Proof{theta_from_jacobi, theta_fixed_point_unique}
\end{remark}
...
\Proof{bridge\_cocycle, bridge\_inverse, bridge\_compose} % ADD fase 1: respaldo Lean del cociclo
...
coincidence. \Proof{bridge_eq_tan_form, bridge_pi_quarter, spectral_angle_tan}
...
for the four items; the Virasoro and brane-bouquet constructions themselves enter cited, \Proof{psi\_functorial, psi\_one}
~\cite{BrownHenneaux86,Witten95}. \Hyp[\cite{BrownHenneaux86, Witten95}]
\end{proposition}
