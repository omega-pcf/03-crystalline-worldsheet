...
\S\ref{ssec:construction} used to fix $\tau=i$.
\ProofWitnessBlock{selfdual\_gaussian\_unique}
...
route \eqref{eq:sigma-basel} gives $n^{2}/6$, which meets $\tfrac32$ only at $n=3$. \ProofWitnessBlock{three\_halves\_unique, arity\_control\_three}
...
Corollary~\ref{cor:bridge-angle} divides by $\cos\alpha$.
\ProofWitnessBlock{spectral\_angle\_tan, spectral\_angle\_tower\_ratio}
...
\end{equation}
\NumericalWitnessBlock{eq:tower-modes}
...
put $55$ at the tower ceiling $\sigma=2n$, where the saturation entropy is largest.
\ProofWitnessBlock{fib\_adjacent\_le\_one, nmodes\_six\_ne\_fib}
\NumericalWitnessBlock{thm:fib-min}
...
it supports are established in \S\ref{sec:dof}.
\ProofWitnessBlock{tower\_ham\_is\_hermitian, tower\_ham\_bounded\_below}
...
The physical content is the identification of the index set, not the arithmetic identity.
\ProofWitnessBlock{regge\_dirichlet\_eq\_zeta, regge\_tower\_is\_euler\_product}
\end{remark}
...
read at $\tau=i$ and at general $t$. \ProofWitnessBlock{theta\_from\_jacobi, theta\_fixed\_point\_unique}
...
generations---which remains structural.
\ProofWitnessBlock{colour\_ratio}
The ultraviolet is regulated in two complementary
...
$\chi(\mathbb{CP}^2)=3=n$
\ProofWitnessBlock{hopf\_latitude, hopf\_from\_clifford}
The same arity-$3$ of
...
$S_3$ symmetry, $A_2$ root system $=\mathfrak{su}(3)$); the same modulus on the
$i/\pi$ side, where $(\tfrac12)!=\tfrac12\sqrt\pi$ fixes $\mu=\tfrac12$ (\S\ref{ssec:construction}),
is the Gauss lattice $\Z[i]$ (\emph{isometry} face: $\tau=i$, $\ell=1$).
\ProofWitnessBlock{eisenstein\_omega, omega\_eigenvalues, a2\_minimal\_vectors\_exactly\_six}
The $\golden$-mediation
$S_3\to\Z_4$ between them preserves $|\Om|=\tfrac12$ and fixes $\tau=i$:
...
\eqref{eq:isometry} requires. The GKPW dictionary
of \eqref{eq:isometry} enters cited~\cite{Maldacena98,GKP98,Witten98}. \ProofWitnessBlock{holographic\_area, hopf\_from\_clifford, hopf\_latitude, G\_AB\_pcf, lambda5\_pcf, R\_scalar\_pcf, BF\_value, omega\_eigenvalues, a2\_minimal\_vectors\_exactly\_six, eisenstein\_omega, hypercube\_card}
\begin{figure}[!htbp]\centering
...
(Remark~\ref{rmk:spectral-origin}): \ProofWitnessBlock{bulk\_boundary\_isometry, spectral\_uniqueness, bulk\_boundary\_isometry}
...
mechanism: at Type~IIA strong coupling the states in the tower are the Kaluza--Klein modes of
...
shared HS/Maldacena signature) are checked numerically (\eqref{eq:obs-swampland} \NumericalWitness{eq:obs-swampland}, \eqref{eq:obs-jacobson} \NumericalWitness{eq:obs-jacobson})
...
coincidence. \ProofWitnessBlock{bridge\_eq\_tan\_form, bridge\_pi\_quarter, spectral\_angle\_tan}
...
for the four items; the Virasoro and brane-bouquet constructions themselves enter cited~\cite{BrownHenneaux86,Witten95}.
\end{proposition}
