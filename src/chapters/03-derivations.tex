...
in the pytest suite \texttt{tests/} (\S\ref{ssec:status}), at $25$-digit precision where
...
(Verified in \texttt{tests/test\_gravity.py}: $H(\tfrac12)=1$, $c=3$, $\GN=\tfrac12$.)
...
\end{enumerate}
(Verified in \texttt{tests/test\_core\_identities.py} and \texttt{tests/test\_tower.py}: $\Gamma(\tfrac12)^{2}=\pi$, Regge poles,
...
\texttt{tests/test\_core\_identities.py}: $|\Om|_{\sigma}=\tfrac12\ \forall\sigma$,
...
shared HS/Maldacena signature) are checked numerically in \texttt{tests/test\_gravity.py}
