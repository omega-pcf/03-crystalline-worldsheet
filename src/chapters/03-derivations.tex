...
\S\ref{ssec:construction} used to fix $\tau=i$. \Proof{selfdual\_gaussian\_unique}
...
carries the fibre.
\ProofBlock{fibre\_monodromy, fibre\_reflection\_order\_two, fibre\_independent\_of\_base}
\end{proposition}
...
route \eqref{eq:sigma-basel} gives $n^{2}/6$, which meets $\tfrac32$ only at $n=3$. \Proof{three\_halves\_unique, arity\_control\_three}
...
Corollary~\ref{cor:bridge-angle} divides by $\cos\alpha$. \Proof{spectral\_angle\_tan, spectral\_angle\_tower\_ratio}
...
put $55$ at the tower ceiling $\sigma=2n$, where the saturation entropy is largest. \Proof{fib\_adjacent\_le\_one, Nmodes\_six\_ne\_fib} \Numeric{thm:fib-min}
...
The physical content is the identification of the index set, not the arithmetic identity. \Proof{regge\_dirichlet\_eq\_zeta, regge\_tower\_is\_euler\_product}
...
read at $\tau=i$ and at general $t$. \Proof{theta\_from\_jacobi, theta\_fixed\_point\_unique}
...
\ProofBlock{R\_scalar\_pcf, Lambda5\_pcf, G\_AB\_pcf, BF\_value}
...
$S(\sigma)=\pi\golden^{\sigma}$ \eqref{eq:tower-modes}. \Proof{conductor\_attribution, four\_cocone, S\_tower}
...
\ProofBlock{eisenstein\_omega, Omega\_eigenvalues, a2\_minimal\_vectors\_exactly\_six};
...
(Remark~\ref{rmk:spectral-origin}): \Proof{bulkBoundary\_isometry, spectral\_uniqueness, bulkBoundary\_isometry}
...
\ProofBlock{certainty\_principle}
...
\ProofBlock{bridge\_cocycle, bridge\_inverse, bridge\_compose} % ADD fase 1: respaldo Lean del cociclo
...
coincidence. \Proof{bridge\_eq\_tan\_form, bridge\_pi\_quarter, spectral\_angle\_tan}
