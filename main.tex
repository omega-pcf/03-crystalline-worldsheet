...
% \Numerical: right-aligned numerical-witness tag (block form, for tier{N} declarations).
% Cites equation labels (stable), not test file paths (implementation).
\newcommand{\Numerical}[1]{\par\noindent\hfill\parbox{\dimexpr\linewidth-1em}{\raggedleft\small\itshape[#1]}}
% \PytestInline: inline test path (used once, in the status section only).
