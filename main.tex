...
% \Lean: always starts its own paragraph, right-aligned.
% After \end{equation} or at the start of a line, this creates the desired layout:
%   equation (N.N)
%   [decl1, decl2]                          ← right-aligned on own line
%   Next paragraph resumes here.
% For inline uses (\tier{P} \Lean{...}), the \tier already starts a new line,
% so \Lean just adds the right-aligned continuation.
\newcommand{\Lean}[1]{\par\noindent\hfill{\small\texttt{[#1]}}\par}
